...
(`internal/loop/watchdog.go`). Only valid adapter-recognized events reset it (the
...
- **The box runs in its own process group for every built-in attempt** (non-nil Ctx →
  ...
  context (`internal/loop/loop.go`); killing coop with `-9` orphans the box — matching a real container
...
- 2026-08-10 — sources repointed: the whole engine moved out of `internal/cli` into `internal/loop`
  (`watchdog.go`/`streamjson*.go`/`ratelimit.go` whole, `commands.go`'s engine half into `loop.go` +
  `iteration.go`). Re-verified every claim against the new files — the move was mechanical and the
  deadlines, activity sinks, and process-group behavior are unchanged.
