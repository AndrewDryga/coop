...
- One task = one commit. Spot unrelated work? → `coop tasks add` (the queue works it soon); only a genuinely large one → `coop backlog add`. Either way, stay on the task you're on.
