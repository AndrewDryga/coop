...
| `coop acp [<target|preset>]` | run as an [ACP](#drive-it-from-zed-acp) agent over stdio (for Zed) — automatically start a signed-in provider, then use the live Preset/Provider/Account selectors; pin a model/effort/account in a target or name a preset; add read-only peers with `--peer <target>` |
...
| `coop tasks watch` | one live board for the canonical tasks (auto-exits when work drains, exits 1 when a queue cannot be read; Ctrl-C anytime) |
| `coop tasks add "<title>"` · `claim [--as <label>] [--pid <n>] [--force]` · `release` · `block [--question <text> --option <text>... --recommendation <text>]` · `unblock` · `done` · `rm` | move one task through its states (moving its folder is the state change); an agent's claim binds to its process so `ls`/`watch` show who holds the task and whether it is still alive, and `coop loop --preflight` releases a claim whose process is gone; `release` returns the task to todo with its handoff notes; `block`'s text options write the whole decision request as the task is parked, and never overwrite a decision somebody already wrote |
...
outside the mounted profile. On a filtered run, Coop's network gateway uses the key without
mounting it or placing it in the box environment. The folders appear on first run, and each
tool's normal user-level config works there as-is — `claude/settings.json`, `codex/config.toml`,
`gemini/settings.json`, `grok/config.toml`. Provider-owned OAuth and access-token files remain a
separate credential family: only the active account is mounted or projected, never the whole
credential store.

Each run receives only the launched agent's credential scope: `coop claude` may mount
`~/.claude`, never the Codex or Gemini homes. A supported API-key run must use
`--egress filtered`; the box gets a random, run-scoped substitute for each key while the reusable
key stays in the gateway. Unsupported keys and launch shapes stop before a box starts.
...
…or use one of the API keys Coop can broker in a filtered run:
...
Without a target, Coop starts the first signed-in provider in Claude, Codex, Gemini, Grok order,
using its default account. The automatic choice is not pinned: select a provider or preset in
the toolbar. If none is signed in, run `coop login <agent>` and reconnect the editor.

To pin the model, reasoning effort, and account, put a target inside `args` —
...
thread. Zed launches `coop acp` (optionally with a target or preset) with the project as cwd; the selected provider runs in the
