...
AGENTS
  ...
  coop <target> --readonly          read the repo, write only to scratch
  coop <target> --bare              Q&A only: no repo, no context, no tools

CREDENTIALS, MODELS & PRESETS
  coop login <agent>                sign in an agent (a subscription)
  coop credentials [<agent>]        the accounts Coop can use
  coop models [<agent>]             the models available to each agent
...
SAFETY — prove the box holds, catch committed secrets
  ...
  coop net                          control network access, see what runs reach
