...
AGENTS
  coop <agent>[:model][@account]    a sandboxed agent (the target grammar)
  coop <preset>                     run a preset interactively (its lead leads)
  coop fusion <agent|preset>        one leads, council members advise
  coop acp <agent|fusion|preset>    serve as an editor agent (ACP; e.g. Zed)
  coop <agent> --peer <peer>…       a read-only second opinion, named peers
...
coop acp <agent|fusion|preset> — serve as an ACP agent over stdio (for editors).
...
coop fusion <agent> --peer <agent>... — one agent leads, council members advise, it synthesizes.
...
coop backlog — park the BIG or not-yet-ready as task folders (.agent/tasks/xx_backlog/).
