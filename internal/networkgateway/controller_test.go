...
func newTestController(t *testing.T, policy egress.Snapshot, apply ApplyRules) *Controller {
	t.Helper()
	clock := testBootClock()
	c, err := NewController(Identity{Clock: clock.Domain(), RunID: strings.Repeat("a", 32), Epoch: strings.Repeat("b", 32), PolicyFingerprint: policy.Fingerprint}, policy, nil, nil, nil, nil, netip.Addr{}, nil, clock, apply)
	if err != nil {
		t.Fatal(err)
	}
	return c
