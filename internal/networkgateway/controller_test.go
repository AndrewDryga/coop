...
func TestTerminalCloseWinsAnInFlightSuccessfulLeaseCommit(t *testing.T) {
	...
	go func() {
		_, err := c.Admit(context.Background(), Lease{Name: "api.example.com", Peer: netip.MustParseAddr("93.184.216.34"), Expires: fixed.Add(time.Minute)})
		admitted <- err
	}()
	select {
	case <-applying:
...
func TestLeaseRulesNeverEmitSubTickTimeoutOrClaimRoundingAsLifetime(t *testing.T) {
	...
	rules, until := leaseRules(map[string]Lease{"short": {Peer: peer, Expires: now.Add(ControllerUpdateTimeout + 19*time.Millisecond)}}, now)
	if strings.Contains(rules, "add element") || len(until) != 0 {
		t.Fatal("sub-tick timeout could become permanent on older kernels")
	}
	rules, until = leaseRules(map[string]Lease{"normal": {Peer: peer, Expires: now.Add(time.Second)}}, now)
	if !strings.Contains(rules, "timeout 750ms") || until[peer] != now.Add(730*time.Millisecond) {
...
func TestControllerInstallsDenyBeforeReadinessAndNeverReinitializes(t *testing.T) {
	...
	for _, required := range []string{"hook input priority 0; policy drop", "hook output priority 0; policy drop", "hook forward priority 0; policy drop",
		"priority -110", "meta nfproto ipv6", "meta skuid 65532 tcp dport 443 ct state established accept", "set leases4", "flags timeout", "size 4096"} {
...
func TestControllerLeaseValidationAndAtomicSharedIPRefresh(t *testing.T) {
	...
	lease := Lease{Name: "api.example.com", Peer: netip.MustParseAddr("93.184.216.34"), Expires: now.Add(10 * time.Second)}
	...
	if len(batches) != 2 || batches[1] != "flush set inet coop_net leases4\nadd element inet coop_net leases4 { 93.184.216.34 timeout 9750ms }\n" {
	...
	other := Lease{Name: "one.services.example.com", Peer: lease.Peer, Expires: now.Add(20 * time.Second)}
	...
	now = now.Add(2 * time.Second)
	third := Lease{Name: "two.services.example.com", Peer: netip.MustParseAddr("1.0.0.1"), Expires: now.Add(5 * time.Second)}
	if _, err := c.Admit(context.Background(), third); err != nil || !strings.Contains(batches[3], "93.184.216.34 timeout 17750ms") {
		t.Fatalf("refresh restarted old TTL: %v %q", err, batches)
	}
	for _, invalid := range []Lease{
		{Name: "other.example.net", Peer: lease.Peer, Expires: now.Add(time.Second)},
		{Name: "API.EXAMPLE.COM", Peer: lease.Peer, Expires: now.Add(time.Second)},
		{Name: lease.Name, Peer: netip.MustParseAddr("169.254.169.254"), Expires: now.Add(time.Second)},
		{Name: lease.Name, Peer: netip.MustParseAddr("::ffff:1.1.1.1"), Expires: now.Add(time.Second)},
		{Name: lease.Name, Peer: lease.Peer, Expires: now},
		{Name: lease.Name, Peer: lease.Peer, Expires: now.Add(MaxDNSTTL + time.Second)},
	} {
		before := len(batches)
...
func TestControllerAcceptedUpdateOutlivesOnlyItsRequestCancellation(t *testing.T) {
	...
	applying = true
	if _, err := c.Admit(ctx, Lease{Name: "api.example.com", Peer: netip.MustParseAddr("93.184.216.34"), Expires: testBootNow().Add(time.Minute)}); err != nil || !c.Ready() {
...
func TestControllerDelayedCommitCannotExtendOldDNSAuthority(t *testing.T) {
	...
	applying = true
	start := now
	lease := Lease{Name: "api.example.com", Peer: netip.MustParseAddr("93.184.216.34"), Expires: start.Add(time.Second)}
	...
	other := Lease{Name: "one.services.example.com", Peer: netip.MustParseAddr("1.0.0.1"), Expires: now.Add(time.Minute)}
