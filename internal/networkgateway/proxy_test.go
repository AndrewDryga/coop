...
func TestProxyHeaderPinsIPv4PortAndCorrelationOnlyTLV(t *testing.T) {
	...
	header, err := ProxyHeader(netip.MustParseAddr("93.184.216.34"), id)
	if err != nil {
		t.Fatal(err)
	}
	if string(header[:12]) != "\r\n\r\n\x00\r\nQUIT\n" || header[12] != 0x21 || header[13] != 0x11 ||
		int(binary.BigEndian.Uint16(header[14:])) != len(header)-16 || binary.BigEndian.Uint16(header[26:]) != 443 ||
	...
	for _, invalid := range []string{"", "-", strings.Repeat("g", 32), strings.Repeat("A", 32), strings.Repeat("a", 31) + "\n"} {
		if _, err := ProxyHeader(netip.MustParseAddr("1.1.1.1"), invalid); err == nil {
			t.Fatalf("accepted hostile correlation ID %q", invalid)
		}
	}
	if _, err := ProxyHeader(netip.MustParseAddr("::ffff:1.1.1.1"), id); err == nil {
