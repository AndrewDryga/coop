...
// the guard must first resolve SNI and obtain a controller lease for this IP.
// PP2_TYPE_UNIQUE_ID is correlation metadata, never a routing override.
func ProxyHeader(peer netip.Addr, flowID string) ([]byte, error) {
	if !peer.IsValid() || !peer.Is4() || len(flowID) != 32 {
