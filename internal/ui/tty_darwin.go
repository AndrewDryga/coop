...
func disableTerminalEcho(f *os.File) (func() error, error) {
	...
	_, _, errno := syscall.Syscall6(syscall.SYS_IOCTL, f.Fd(), syscall.TIOCGETA, uintptr(unsafe.Pointer(&old)), 0, 0, 0)
	if errno != 0 {
		return nil, errno
	}
	next := old
	next.Lflag &^= syscall.ECHO
	_, _, errno = syscall.Syscall6(syscall.SYS_IOCTL, f.Fd(), syscall.TIOCSETA, uintptr(unsafe.Pointer(&next)), 0, 0, 0)
	if errno != 0 {
		return nil, errno
	}
	return func() error {
		_, _, errno := syscall.Syscall6(syscall.SYS_IOCTL, f.Fd(), syscall.TIOCSETA, uintptr(unsafe.Pointer(&old)), 0, 0, 0)
		if errno != 0 {
