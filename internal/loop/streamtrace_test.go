...
func TestIterationStreamTraceGatesAndWarnsOnce(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			repo := t.TempDir()
			c := &Control{cfg: &config.Config{StreamTrace: tc.cfg}, runID: tc.runID}
			raw, rendered, close := c.iterationStreamTrace(repo, "claude", tc.streaming)
			close()
			if raw != nil || rendered != nil || c.streamSeq != 0 {
				t.Fatalf("trace = (%v, %v), seq=%d; want nil, nil, 0", raw, rendered, c.streamSeq)
			}
	...
	c := &Control{cfg: &config.Config{StreamTrace: true}, runID: "run"}
	var warnings []string
	ui.SetLiveSink(func(s string) { warnings = append(warnings, s) })
	defer ui.SetLiveSink(nil)
	for i := 0; i < 2; i++ {
		raw, rendered, close := c.iterationStreamTrace(repo, "claude", true)
		close()
		if raw != nil || rendered != nil {
			t.Fatalf("failed open %d returned non-nil writers", i+1)
		}
	}
	if !c.streamOff || c.streamSeq != 1 {
		t.Errorf("failure state = off %v seq %d, want true/1", c.streamOff, c.streamSeq)
	}
