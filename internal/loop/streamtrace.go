...
func (c *Control) iterationStreamTrace(repo, agent string, streaming bool) (raw, rendered io.Writer, close func()) {
	noop := func() {}
	if !streaming || !c.cfg.StreamTrace || c.runID == "" || c.streamOff {
		return nil, nil, noop
	}
	c.streamSeq++
	dir := filepath.Join(repo, ".agent", "runs", c.runID+".streams")
	raw, rendered, close, err := openStreamTrace(repo, c.runID, c.streamSeq, agent)
	if err != nil {
		c.streamOff = true
