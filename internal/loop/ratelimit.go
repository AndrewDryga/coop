...
func sleepForLimitAt(wait time.Duration, resetAt time.Time, wake <-chan struct{}, nowFn func() time.Time) {
	...
	if resetAt.IsZero() {
		...
		ui.Note("  Continuing in %s, at %s.", humanWait(wait), resetClock(resetAt))
	}
	...
	ladder.WaitUntilWall(start.Add(wait), ladder.LimitTickCap, nowFn, wake, func(remaining time.Duration) {
		if t := nowFn(); t.Sub(last) >= narrate {
			last = t
			ui.Note("  %s remaining", humanWait(remaining))
		}
	})
}

// loopAction is what Run should do after one iteration.
type loopAction int
...
// signoffRoundOutcome decides what Run does after a signoff pass, given the just-finished round
...
// counters in place, and returns the action Run should take (with the pause and
