...
func LoopWorkPrompt(repo, assignedRoot, assignedID, agent string, peers []agents.Target, p *preset.Preset, auditReopen bool) string {
	...
	discoveryPolicy := "If you SPOT a SEPARATE task while working (not part of this one), do NOT fold it into your commit: file it with tasks_propose as kind task, with an acceptance you can state in a line, and a later iteration works it. Only the genuinely LARGE is filed as kind backlog instead — work no single iteration could finish, or a spec-sized idea a human must scope first. \"It needs a design\" is not a reason to park something you could state in a line; when the call is close, file it as a task. Never create a task folder or a proposal file by hand."
	instructions := strings.Join([]string{
		...
		"Put disposable but resumable scratch work (patches, generated files) under the assigned task's tmp/ directory; it survives interruption and blocked transitions but Coop removes it on completion. Create worktrees or branches only when explicitly allowed by the project and human. Before finishing, promote anything a reviewer or future maintainer needs to the task's durable artifacts/ directory.",
