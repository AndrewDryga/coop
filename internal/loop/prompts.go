...
func LoopWorkPrompt(repo, assignedRoot, assignedID, agent string, peers []agents.Target, p *preset.Preset, auditReopen bool) string {
	...
	instructions := strings.Join([]string{
		...
		"Do not end your turn while any gate, consult, delegate, or other background job you started remains live; use supported completion tools to wait for and inspect its result, not long blind sleeps. If a background wait times out, call its completion tool again until the job exits — saying it is still running and ending the turn forces the next attempt to rerun the check. Rerun an ambiguous gate in the foreground. Cleanup uses the project's lifecycle command or exact verified owned process/resource identities, never broad name-pattern kills; verify the resource is stopped/absent and report cleanup failure. Use unique, create-only scratch database names; never drop a pre-existing unknown database.",
