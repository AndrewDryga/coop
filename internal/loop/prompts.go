...
func LoopWorkPrompt(repo, assignedRoot, assignedID, agent string, peers []agents.Target, p *preset.Preset, auditReopen bool) string {
	commitPolicy := "Do the work, run the gate, then commit your work — END the commit message with a trailer line `Coop-Task: <task-id>` (the task id is its folder name), so the harness can bind the commit to the task, resume correctly if interrupted, and reconcile the queue after a fork merge. If the task is already fully satisfied by existing implementation, do not fabricate edits or an empty commit: verify every actual requirement and use the explicit no-change completion below."
	...
	completionPolicy := "For implemented work, AFTER the commit refresh state.md one last time with tasks_update_state while the task is still in 10_in_progress/, then call tasks_complete on your task as the final action, with its task id. If existing implementation already satisfies every requirement, call tasks_complete with outcome `already_satisfied`, a concrete reason, and evidence naming the existing task/commit plus verification; no commit is needed. An investigation task may similarly use `could_not_reproduce` or `wont_fix` after reasonable focused attempts, with exact evidence and remaining uncertainty; never say fixed. A human choice always uses tasks_block instead. tasks_complete sets Status to complete and Next action to none and moves the folder into 99_done/; write nothing more inside that task folder after it."
	if auditReopen {
		commitPolicy = "Do the work and run the gate. This task is host-authorized audit rework: if independent verification shows the finding is false, do NOT create, amend, or rewrite any commit — complete it with zero new commits. If the finding is real, add exactly one real repair commit WITHOUT a Coop-Task or Coop-Recovery trailer, preserving the reviewed history; the host audit authority binds it to this task."
		citationPolicy = "If you mention the repair in state.md or log.md, call it the host-authorized repair for the task, NOT by SHA — host signing can change its SHA."
		completionPolicy = "AFTER the gate — and after the one unbound repair commit only when a real fix was required — refresh state.md one last time with tasks_update_state while the task is still in 10_in_progress/ (preserve the useful Done so far and Traps), then call tasks_complete on your task as the final action: it sets Status to complete and Next action to none and moves the folder into 99_done/; write nothing more inside that task folder after it."
	}
	...
	instructions := strings.Join([]string{
		...
		"The assigned project directory is %s. For source and test commands, `cd` to that absolute path and use project-relative paths; never add the subproject name again. Use repository-root absolute paths only when a command truly spans projects.",
		...
		"Work task %s, already claimed in 10_in_progress/. Read it with tasks_get — its task.md and state.md (the resume note: where prior work stopped, the next action, and traps) — then inspect `git status --short`, `git diff`, and `git diff --cached`. Treat a detailed state.md as the handoff: continue from its Next action instead of repeating completed research unless the source or working diff has changed. Establish change ownership before resuming. Preserve unrelated or uncertain staged, unstaged, and untracked work; never blanket-restore/reset the checkout. Discard only verified task-owned paths/hunks when authorized. Before mutation tests, preflight ownership and capture original bytes/index state before arming any restoration hook; restore that captured state, not HEAD.",
		"Before starting a consult or delegate, check state.md and any task-local output it names. Reuse a completed result for the same question on the unchanged diff; if it says the job is running, inspect or wait for that output before launching a replacement. Retry only a failed, missing, or stale peer job.",
		...
		"As you work, keep that task's state.md current with tasks_update_state — a small snapshot of the status, what is done, the next action, and any traps. Send only the fields that changed; omitted fields are preserved. Refresh it before each commit and before you pause; record your reasoning with tasks_append_log, and keep its ## Subtasks checklist truthful with tasks_set_subtasks (send the whole list: add, refine, reorder, check off).",
		...
		"Once the task contract and relevant paths are known, stop surveying sibling code just in case; implement the smallest acceptance-complete change and verify it.",
		"Run the final gate in the foreground. Only background it when you have independent work to do. If a command tool returns a running-job identifier, wait on that job using supported completion tools, not long blind sleeps. If a background wait times out, call its completion tool again until the job exits. Do not end your turn while any gate, consult, delegate, or other job you started remains live. Rerun an ambiguous gate in the foreground. Cleanup uses the project's lifecycle command or exact verified owned process/resource identities, never broad name-pattern kills; verify the resource is stopped/absent and report cleanup failure. Use unique, create-only scratch database names; never drop a pre-existing unknown database.",
		...
		"Before completion, put a compact `Verification (worker-reported):` block in Done so far: the check cwd; each exact command and result; whether it ran after the final edits; every skipped or missing required check; and an optional durable full-log path under artifacts/. Do not paste full logs. Refresh this handoff after any review-driven code change.",
		...
		"For an incidental flaky failure, make reasonable focused reproduction attempts. If it does not reproduce and this task's required checks pass, search for an existing investigation task, then file exactly one if needed with the original failure, command and cwd, seed/conditions, relevant logs, attempts, and remaining uncertainty; continue this task. A later investigation may close as could_not_reproduce or wont_fix without a commit, but never as fixed, and must not spawn another identical investigation task.",
		commitPolicy,
		citationPolicy,
		completionPolicy,
	...
	return loopPeerCapabilities(agent, peers, p) + "\n\n" + fmt.Sprintf(instructions,
		...
		filepath.Dir(filepath.Dir(absQueuePath(repo, assignedRoot))),
...
// then consumes the worker's compact verification handoff — reopening anything short of
...
const defaultSignoffPrompt = "Review pass — you are the SENIOR REVIEWER for work done unattended overnight. Make sure every shipped task is CORRECT, meets its stated goal, follows this repo's standards, and is genuinely polished — not merely \"the gate is green.\" You do NOT fix code or make commits: when something falls short you REOPEN the task with a SPECIFIC, actionable note, and the work loop fixes it next round. Be demanding — the bar is work you'd merge with no changes.\n" +
	"The project contract is normally already loaded in your context; do not reread it unless it is absent. Read only the rule cards relevant to the changed files. `coop` is not installed in this review box, so read the task folders at the paths above directly. Do not probe a service merely to reconfirm an explicitly reported unavailable dependency; run a focused probe only when it tests a concrete new finding.\n" +
	...
	"3. Tested for real — inspect whether its tests exercise the FAILURE/edge path and actually cover the new behavior. Trust the worker-reported execution handoff, but reopen the task when required verification is missing, failed, or stale after later edits. Name the exact check the worker must run.\n" +
	...
	"5. Bookkeeping — exactly one reachable commit implementing it exists in git log (find it by its Coop-Task: <task-id> trailer, not by a SHA in notes), unless its task record explicitly closes a verified no-change outcome with reason and evidence. A similar title alone is not evidence. A final state.md is present and the queue is internally consistent. Status must be complete and Next action must be none. Coop finalizes those lifecycle values before review; never edit a task in place under 99_done/. If they are unexpectedly wrong, report a completion-integrity defect and say that no implementation change is required. Task-local tmp/ was disposable and has been removed before this review; evidence that needed to survive completion belongs in artifacts/.\n" +
	"Do not run tests, gates, services, or other executable probes during review. Inspect code, tests, diffs, task records, and existing log excerpts. If another check or experiment is needed, reopen the responsible task with that exact request; the worker runs it and refreshes the handoff.\n" +
...
func loopSignoffPrompt(repo string, queues []string, appendPrompt string, finished []string) string {
	...
	b.WriteString("\n\n")
	b.WriteString(auditEvidencePrompt(taskIDsOf(finished)))
	b.WriteString("\n\n")
...
func reviewContextFooter(repo string, queues []string) string {
	return fmt.Sprintf("Context: the task queue(s) are at %s and the project contract is %s. Task lifecycle is report-only in every review: do not edit anything under the task queues or move task folders. Source is read-only unless this stage explicitly grants writes: repo; even then, queue lifecycle remains report-only. Report defects only in your structured evidence and terminal receipt. Coop validates the complete proposal, then acquires host-side task authority and applies exact-subject reopens with the failure note and resume state. A missing, malformed, interrupted, or out-of-scope proposal mutates nothing.",
		...
		" You are the authoritative review for this stage: do NOT invoke the review-board skill or spawn another review board. Do not execute tests, gates, services, or other probes. Inspect the code, tests, history, worker-reported verification, and existing log excerpts. When execution evidence is missing, failed, or stale, reopen the task and name the exact check the worker must run." +
...
func auditEvidencePrompt(subjects []string) string {
	ids := slices.Clone(subjects)
	slices.Sort(ids)
	var b strings.Builder
	b.WriteString("FINAL RESPONSE FORMAT: return only the following plain-text lines. Do not use a Markdown fence, bullets, headings, blank lines, or prose before, between, or after them. Replace angle-bracket fields, but copy every task ID exactly.\n")
	for _, id := range ids {
		fmt.Fprintf(&b, "AUDIT EVIDENCE — %s — gate: <worker-reported command/result, or missing/failed/stale> — findings: <concrete unresolved findings, or none>\n", id)
	}
	b.WriteString("REVIEW COMPLETE — PASS — reopened: none\n")
	b.WriteString("If any task fails, use the same evidence lines, put a concrete finding instead of `none` for each failing task, and replace the receipt with:\n")
	b.WriteString("REVIEW COMPLETE — FAIL — reopened: <failing task IDs sorted with commas and no spaces; only IDs named above>\n")
	b.WriteString("The findings field may be exactly `none` or `none (<short annotation>)`; prose that merely begins with `none` is a finding. Every reopened ID must have a concrete finding and every concrete finding must be reopened. Coop validates the exact IDs and applies any reopen on the host.")
	return b.String()
}

func seedReviewPrompt(prompt string, blocks ...string) string {
	const marker = "FINAL RESPONSE FORMAT:"
	at := strings.Index(prompt, marker)
	if at < 0 {
		return prompt + strings.Join(blocks, "")
	}
	return prompt[:at] + strings.Join(blocks, "") + "\n\n" + prompt[at:]
}
...
const defaultProtectedBetweenPrompt = "Audit ONLY the protected gate change named above. Verify from the committed diff and the worker's final gate report that it preserves or strengthens enforcement rather than removing an assertion, disabling a hook, or relaxing what counts as green. Do not run the gate yourself; reopen the task with the concrete weakness or exact missing check if it does not pass that bar."
