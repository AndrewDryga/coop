...
// then consumes Coop's exact-tree gate receipt — reopening anything short of
...
const defaultSignoffPrompt = "Review pass — you are the SENIOR REVIEWER for work done unattended overnight. Make sure every shipped task is CORRECT, meets its stated goal, follows this repo's standards, and is genuinely polished — not merely \"the gate is green.\" You do NOT fix code or make commits: when something falls short you REOPEN the task with a SPECIFIC, actionable note, and the work loop fixes it next round. Be demanding — the bar is work you'd merge with no changes.\n" +
	...
	"Use the Coop-owned exact-tree gate receipt included below. Do not rerun that matching broad gate; run only targeted probes needed to test a new review hypothesis. If the receipt failed, reopen the responsible task(s) — the most-recently-done whose commit plausibly caused it — with the failure.\n" +
...
func auditEvidencePrompt(subjects []string) string {
	...
	for _, id := range ids {
		fmt.Fprintf(&b, "AUDIT EVIDENCE — %s — gate: <test actually run, or not run with why> — findings: <concrete unresolved findings, or none>\n", id)
	}
