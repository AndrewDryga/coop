...
func TestAggregateDuplicateTaskIDs(t *testing.T) {
	...
	if got, want := mustAggregateDuplicateTaskIDs(t, hosts), []string{"actionable", "archived", "blocked"}; !slices.Equal(got, want) {
		t.Fatalf("aggregate duplicate ids = %v, want %v", got, want)
	}
	if got, want := mustNonArchivedDuplicateTaskIDs(t, hosts), []string{"actionable", "blocked"}; !slices.Equal(got, want) {
...
func TestCompletionWindowAndRestoreRespectForeignLease(t *testing.T) {
	root := t.TempDir()
	old := taskWithCompletedChecklist(t, root, StateDone, "old")
	assigned := taskWithCompletedChecklist(t, root, StateInProgress, "assigned")
	rogue := taskWithCompletedChecklist(t, root, StateInProgress, "rogue")
	spoofed := taskWithCompletedChecklist(t, root, StateInProgress, "spoofed")
	foreign := taskWithCompletedChecklist(t, root, StateInProgress, "foreign")
	finalized := taskWithCompletedChecklist(t, root, StateInProgress, "finalized")
...
func TestCompletionWindowDetectsNewArchiveAndReceiptClearedSamePath(t *testing.T) {
	root := t.TempDir()
	legacy := taskWithCompletedChecklist(t, root, StateDone, "legacy")
	...
	rogue := taskWithCompletedChecklist(t, root, StateInProgress, "rogue-in-window")
...
func TestReviewCompletionWindowDetectsNewReceiptGeneration(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateDone, "recompleted-during-review")
	...
	reopened, _ := mustCurrentTask(t, root, archived.ID)
	if err := CompleteTrustedTask(root, reopened); err != nil {
		t.Fatal(err)
	}
	recompleted, _ := mustCurrentTask(t, root, archived.ID)
...
func TestReviewCompletionWindowRejectsRawSameInodeOutAndBack(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateDone, "raw-review-recompletion")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestWorkCompletionWindowRejectsRawSameInodeOutAndBack(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateDone, "raw-work-recompletion")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestWorkCompletionWindowWaitsToInvalidateStaleReceipt(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateDone, "contended-work-recompletion")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestWorkCompletionWindowAcceptsHostReceiptedForeignArchivedDeparture(t *testing.T) {
	root := t.TempDir()
	assigned := taskWithCompletedChecklist(t, root, StateInProgress, "work-subject")
	archived := taskWithCompletedChecklist(t, root, StateDone, "host-reopened-archive")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestWorkCompletionWindowRejectsSubjectAndWrongDepartureRecord(t *testing.T) {
	t.Run("subject", func(t *testing.T) {
		root := t.TempDir()
		assigned := taskWithCompletedChecklist(t, root, StateDone, "work-subject-departure")
		if err := CompleteTrustedTask(root, assigned); err != nil {
			t.Fatal(err)
		}
		assigned, _ = mustCurrentTask(t, root, assigned.ID)
		windows, err := BeginWorkCompletionWindows([]string{root}, assigned.ID)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("wrong nonce", func(t *testing.T) {
		root := t.TempDir()
		assigned := taskWithCompletedChecklist(t, root, StateInProgress, "work-subject-wrong-nonce")
		archived := taskWithCompletedChecklist(t, root, StateDone, "wrong-nonce-archive")
		if err := CompleteTrustedTask(root, archived); err != nil {
			t.Fatal(err)
		}
		archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestWorkCompletionWindowReplayAcceptsHostReceiptedForeignDeparture(t *testing.T) {
	root := t.TempDir()
	assigned := taskWithCompletedChecklist(t, root, StateInProgress, "replay-work-subject")
	archived := taskWithCompletedChecklist(t, root, StateDone, "replay-host-reopened")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestReviewCompletionWindowRejectsInPlaceArchiveMutation(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateDone, "mutated-review-archive")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestReviewCompletionWindowRejectsDirectTaskReopen(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateDone, "direct-review-reopen")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestReviewCompletionWindowRejectsReplacedSubject(t *testing.T) {
	root := t.TempDir()
	subject := taskWithCompletedChecklist(t, root, StateDone, "replaced-review-subject")
	if err := CompleteTrustedTask(root, subject); err != nil {
		t.Fatal(err)
	}
	subject, _ = mustCurrentTask(t, root, subject.ID)
...
func TestReviewCompletionWindowReportsConcurrentHostCompletion(t *testing.T) {
	...
	subject, _ = mustCurrentTask(t, root, subject.ID)
	foreign := taskWithCompletedChecklist(t, root, StateTodo, "foreign-host-completion")
	windows, err := BeginReviewCompletionWindows([]string{root}, []string{subject.ID})
	if err != nil {
		t.Fatal(err)
	}
	...
	completed, ok := mustCurrentTask(t, root, foreign.ID)
	...
	if kept, _ := mustCurrentTask(t, root, subject.ID); kept.State != StateDone {
...
func TestReviewCompletionWindowReportsCompletionAfterInitialAudit(t *testing.T) {
	root := t.TempDir()
	subject := taskWithCompletedChecklist(t, root, StateDone, "handoff-review-subject")
	foreign := taskWithCompletedChecklist(t, root, StateTodo, "handoff-host-completion")
	windows, err := BeginReviewCompletionWindows([]string{root}, []string{subject.ID})
	if err != nil {
		t.Fatal(err)
	}
...
func TestReviewCompletionWindowSubjectStaysStrictBesideConcurrentActivity(t *testing.T) {
	root := t.TempDir()
	subject := taskWithCompletedChecklist(t, root, StateDone, "strict-review-subject")
	if err := CompleteTrustedTask(root, subject); err != nil {
		t.Fatal(err)
	}
	subject, _ = mustCurrentTask(t, root, subject.ID)
	foreign := taskWithCompletedChecklist(t, root, StateTodo, "foreign-beside-subject")
	windows, err := BeginReviewCompletionWindows([]string{root}, []string{subject.ID})
	if err != nil {
		t.Fatal(err)
	}
	...
	if done, _ := mustCurrentTask(t, root, foreign.ID); done.State != StateDone {
...
func TestReviewCompletionWindowRejectsRawNonSubjectCompletion(t *testing.T) {
	root := t.TempDir()
	subject := taskWithCompletedChecklist(t, root, StateDone, "raw-review-subject")
	if err := CompleteTrustedTask(root, subject); err != nil {
		t.Fatal(err)
	}
	rogue := taskWithCompletedChecklist(t, root, StateTodo, "raw-review-completion")
	windows, err := BeginReviewCompletionWindows([]string{root}, []string{subject.ID})
	if err != nil {
		t.Fatal(err)
	}
...
func TestReviewCompletionWindowWithoutSubjectsStaysStrict(t *testing.T) {
	root := t.TempDir()
	foreign := taskWithCompletedChecklist(t, root, StateTodo, "foreign-without-subjects")
...
func TestTrustedTaskReopenRollsBackMetadataAndReceipt(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			root := t.TempDir()
			task := taskWithCompletedChecklist(t, root, StateDone, "atomic-review-reopen")
			if tc.complete {
				...
				task, _ = mustCurrentTask(t, root, task.ID)
			}
...
func TestTrustedTaskReopenRollsBackEverySubject(t *testing.T) {
	...
	for _, id := range []string{"atomic-review-a", "atomic-review-b"} {
		task := taskWithCompletedChecklist(t, root, StateDone, id)
		...
		task, _ = mustCurrentTask(t, root, id)
...
func TestStaleReceiptClearDoesNotEraseFreshGeneration(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateDone, "fresh-receipt")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestCompletionWindowAuditsBusyReceiptBaseline(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateDone, "busy-baseline")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestTrustedReopenMoveFailureRestoresExactReceipt(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateDone, "failed-trusted-reopen")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
	...
	if _, err := windows.candidates(); err == nil || !strings.Contains(err.Error(), StateInProgress) {
		t.Fatalf("unreadable lifecycle state did not stop completion audit: %v", err)
	}
	if err := os.Remove(filepath.Join(root, StateInProgress)); err != nil {
		t.Fatal(err)
	}
	if err := os.Mkdir(filepath.Join(root, StateInProgress), 0o755); err != nil {
		t.Fatal(err)
	}
	if candidates, err := windows.candidates(); err != nil || len(candidates) != 0 {
		t.Fatalf("repaired failed reopen looked like a completion generation: %#v, %v", candidates, err)
	}
	if err := windows.Close(); err != nil {
		t.Fatal(err)
	}
...
func TestTrustedReopenCrashWindowRestoresClearedReceipt(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateDone, "crashed-trusted-reopen")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestCompletionWindowReplayLeavesChangedBaselineArchivesDone(t *testing.T) {
	...
	trustedArchive := func(id string) Item {
		t.Helper()
		task := taskWithCompletedChecklist(t, root, StateInProgress, id)
		if err := CompleteTrustedTask(root, task); err != nil {
			t.Fatal(err)
		}
		task, _ = mustCurrentTask(t, root, id)
	...
	rogue := taskWithCompletedChecklist(t, root, StateInProgress, "new-unowned-completion")
	...
	for _, archived := range []Item{first, second} {
		current, ok := mustCurrentTask(t, root, archived.ID)
		if !ok || current.State != StateDone {
	...
	current, ok := mustCurrentTask(t, root, rogue.ID)
	if !ok || current.State != StateInProgress {
...
func TestCompletionWindowReplayArrivalPrecedesLaterMutation(t *testing.T) {
	...
	rogue, _ = mustCurrentTask(t, root, rogue.ID)
...
func TestCompletionWindowReplayOwnedArrivalDoesNotHideLaterMutation(t *testing.T) {
	root := t.TempDir()
	arrival := taskWithCompletedChecklist(t, root, StateInProgress, "owned-arrival-before-mutation")
	older, err := BeginCompletionWindows([]string{root})
	if err != nil {
		t.Fatal(err)
	}
	...
	arrival, _ = mustCurrentTask(t, root, arrival.ID)
	newer, err := BeginCompletionWindows([]string{root})
	if err != nil {
		t.Fatal(err)
	}
	...
	current, ok := mustCurrentTask(t, root, arrival.ID)
	if !ok || current.State != StateDone {
...
func TestCompletionWindowReplayLeavesReplacedBaselineArchiveDone(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateInProgress, "replaced-baseline")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
	...
	replacement := taskWithCompletedChecklist(t, root, StateDone, archived.ID)
...
func TestCompletionWindowReplayLeavesBusyBaselineWithoutReceiptDone(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateInProgress, "busy-baseline-replay")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestCompletionWindowReplayResumesPersistedBaselineMutation(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateInProgress, "persisted-baseline-mutation")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestCompletionWindowReplayFreshReceiptSupersedesPersistedMutation(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateInProgress, "recompleted-marked-baseline")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestCompletionWindowReplayRetainsMarkedMutationWhileReceiptOwned(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateInProgress, "owned-marked-baseline-mutation")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestBaselineMutationLockRejectsOwnerAcquiredAfterClassification(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateInProgress, "post-classification-owner")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestCompletionWindowRecoveryDetectsSameIDMetadataHandoff(t *testing.T) {
	root := t.TempDir()
	archived := taskWithCompletedChecklist(t, root, StateInProgress, "same-id-metadata-handoff")
	if err := CompleteTrustedTask(root, archived); err != nil {
		t.Fatal(err)
	}
	archived, _ = mustCurrentTask(t, root, archived.ID)
...
func TestReviewCompletionWindowReplayRejectsSubjectRecompletion(t *testing.T) {
	root := t.TempDir()
	subject := taskWithCompletedChecklist(t, root, StateDone, "recompleted-review-subject")
	if err := CompleteTrustedTask(root, subject); err != nil {
		t.Fatal(err)
	}
	subject, _ = mustCurrentTask(t, root, subject.ID)
	...
	reopened, _ := mustCurrentTask(t, root, subject.ID)
	if err := CompleteTrustedTask(root, reopened); err != nil {
		t.Fatal(err)
	}
	if err := unlockLeaseFile(windows.windows[0].live); err != nil {
...
func TestSubjectFreeReviewCompletionWindowReplayStaysStrict(t *testing.T) {
	root := t.TempDir()
	foreign := taskWithCompletedChecklist(t, root, StateTodo, "subject-free-review-completion")
	windows, err := BeginReviewCompletionWindows([]string{root}, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReviewCompletionWindowReplayPreservesConcurrentHostCompletion(t *testing.T) {
	root := t.TempDir()
	subject := taskWithCompletedChecklist(t, root, StateDone, "crashed-review-subject")
	if err := CompleteTrustedTask(root, subject); err != nil {
		t.Fatal(err)
	}
	subject, _ = mustCurrentTask(t, root, subject.ID)
	foreign := taskWithCompletedChecklist(t, root, StateTodo, "crashed-review-foreign")
	windows, err := BeginReviewCompletionWindows([]string{root}, []string{subject.ID})
	if err != nil {
		t.Fatal(err)
	}
	...
	done, ok := mustCurrentTask(t, root, foreign.ID)
...
func TestReconcileInterruptedCompletions(t *testing.T) {
	...
	t.Run("active completion lease is untouched", func(t *testing.T) {
		...
		host := filepath.Join(repo, TasksRoot)
		item := taskWithCompletedChecklist(t, host, StateInProgress, id)
	...
	t.Run("recovery lock covers rename and bookkeeping window", func(t *testing.T) {
		...
		seedDone(t, repo, id)
		host := filepath.Join(repo, TasksRoot)
		item := mustReadTaskTree(t, host)[0]
		...
		moved := mustReadTaskTree(t, host)[0]
...
func TestAssignLoopTaskClaimsBeforeReturningAndCanBlock(t *testing.T) {
	...
	if _, active := mustQueueProgress(t, []string{q}); active != got.Item.Title {
...
func TestCaptureAuditReopenRecordsCompleteOrderedHistory(t *testing.T) {
	...
	if record.Version != auditReopenVersion || record.BaselineHead != head ||
		record.History == nil || len(record.History) != 3 {
...
type blockedAuditRebaseFixture struct {
...
func newBlockedAuditRebaseFixture(t *testing.T) blockedAuditRebaseFixture {
	t.Helper()
	repo, git := gitRepo(t)
	...
	git("add", "b.txt")
	git("commit", "-q", "-m", "B implementation\n\nCoop-Task: blocked-rebase-descendant")
	...
	git("cherry-pick", b)
	return blockedAuditRebaseFixture{
...
func TestBlockedAuditUnblockRebasesStaleAuthority(t *testing.T) {
	f := newBlockedAuditRebaseFixture(t)
	...
	current, ok := mustCurrentTask(t, f.root, f.id)
	if !ok || current.State != StateTodo {
		t.Fatalf("rebased unblock task = %#v, ok=%v", current, ok)
	}
	got, ok, err := ReadAuditReopenRecord(f.root, f.id)
	if err != nil || !ok {
		t.Fatalf("read rebased authority: ok=%v err=%v", ok, err)
	}
...
func TestRebaseBlockedAuditReopenAcceptsRootSubjectReplay(t *testing.T) {
	repo, git := gitRepo(t)
	id := "blocked-root-transition"
	if err := os.WriteFile(filepath.Join(repo, "a.txt"), []byte("A\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	git("add", "a.txt")
	...
	replacement, err := rebaseBlockedAuditReopen(repo, head, id, record)
	if err != nil {
		t.Fatal(err)
	}
	if !AuditReopenCurrentValid(repo, head, id, replacement) {
		t.Fatal("root subject replay produced invalid rebased authority")
	}
}
...
func TestRebaseBlockedAuditReopenAcceptsMergeSubjectParent(t *testing.T) {
	repo, git := gitRepo(t)
	id := "blocked-merge-parent-transition"
	git("commit", "-q", "--allow-empty", "-m", "root")
	root := gitOut(repo, "rev-parse", "HEAD")
	branch := gitOut(repo, "branch", "--show-current")
	git("checkout", "-q", "-b", "parent-side", root)
	...
	if !AuditReopenCurrentValid(repo, head, id, replacement) {
		t.Fatal("merge-parent replay produced invalid rebased authority")
	}
}

func TestBlockedAuditUnblockFailsClosed(t *testing.T) {
	assertBlocked := func(t *testing.T, f blockedAuditRebaseFixture) {
		t.Helper()
		current, ok := mustCurrentTask(t, f.root, f.id)
	...
	t.Run("tampered subject", func(t *testing.T) {
		...
		if mustDecisionResolved(t, filepath.Join(f.task.Dir, "decision.md")) {
	...
	t.Run("replaced generation", func(t *testing.T) {
		...
		transition, err := lockBlockedAuditReopenUnblock(f.root, f.task, f.record)
		if transition != nil {
			_ = transition.finish(nil)
		}
		if err == nil {
	...
	t.Run("raw move does not rebase", func(t *testing.T) {
		...
		if auditReopenCompletionValid(f.repo, head, head, f.id, got) {
			t.Fatal("raw move silently rebased stale authority")
		}

		base := head
		subjects := CommitsForTask(f.repo, head, f.id)
		descendants := CommitsForTask(f.repo, head, "blocked-rebase-descendant")
	...
	t.Run("move failure leaves authority stale", func(t *testing.T) {
		f := newBlockedAuditRebaseFixture(t)
		transition, err := prepareBlockedAuditReopenUnblock(f.root, f.task)
		if err != nil {
			t.Fatal(err)
		}
		...
		if err := moveBlockedAuditUnblock(f.root, f.task, transition); err == nil {
	...
	t.Run("authority write failure restores blocked folder", func(t *testing.T) {
		...
		transition.replacement.Generation = "invalid-replacement-generation"
		if err := moveBlockedAuditUnblock(f.root, f.task, transition); err == nil {
	...
	t.Run("rollback failure reports unknown state", func(t *testing.T) {
		...
		stageErr := &unblockStageError{
			...
			err: transition.finish(errors.Join(persistErr, rollbackErr)),
		}
	...
	t.Run("crash boundary after move cannot grant authority", func(t *testing.T) {
		...
		if got.Version != auditReopenPendingVersion || got.Version == auditReopenVersion {
			t.Fatalf("pending record version = %d, want non-authorizing %d", got.Version, auditReopenPendingVersion)
		}
		relabeled := got
		relabeled.Version = auditReopenVersion
		if validateAuditReopenRecord(relabeled, f.id) == nil {
			t.Fatal("pending v4 record relabeled as active v3 was accepted")
		}
	...
	t.Run("pending marker plus raw move cannot activate on lease", func(t *testing.T) {
		...
		todo, ok := mustCurrentTask(t, f.root, f.id)
		if !ok {
		...
		inProgress, ok := mustCurrentTask(t, f.root, f.id)
		if !ok {
...
func TestParkStaleAuditReopenPreservesPriorDecision(t *testing.T) {
	...
	if mustDecisionResolved(t, filepath.Join(blockedDir, "decision.md")) {
...
func TestUnblockResolved(t *testing.T) {
	...
	ids := mustUnblockResolved(t, []string{root})
...
func TestUnblockResolvedDoesNotRebaseAuditAuthority(t *testing.T) {
	root := t.TempDir()
	authorityRoot := t.TempDir()
	t.Setenv(TestLeaseAuthorityRootEnv, authorityRoot)
...
func TestAlreadyCommittedInProgress(t *testing.T) {
	...
	if got := mustAlreadyCommittedInProgress(t, repo, hosts); len(got) != 0 {
	...
	got := mustAlreadyCommittedInProgress(t, repo, hosts)
	if len(got) != 1 {
