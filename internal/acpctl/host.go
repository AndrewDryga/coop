...
// rotation.go's account/ladder expansion and modelscache.go's cache write are POLICY that stays
// with their caller (the loop, the ACP control, and the sessions service each hold their own — see
// internal/ladder's package comment), so this package injects the exact cli functions rather than
// re-implement or import them.
//
