...
coop_failure_permanent() {
	...
	LC_ALL=C grep -Eiq 'no such file or directory|command not found|file name too long|unknown (option|model)|unrecognized option|invalid (argument|model)|not signed in|credential[^:]* (missing|unavailable|invalid)' "$@" 2>/dev/null
}
