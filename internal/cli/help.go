...
func printHelp(cfg *config.Config) {
	...
	row("coop fork <name> [agent] [--loop]", "open/resume a fork (clone) + run an agent (-d detaches a loop)")
	row("coop fork ls|logs|review|merge|rm", "list · tail · diff · merge back · discard your forks")
