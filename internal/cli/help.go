...
var commandHelp = map[string]string{
  ...
  Usage: coop models [claude|codex|gemini] [--refresh]

  One line per agent with its models, then how to pick one. Each list is either LIVE
  (dim '(live)') from a per-agent cache, or the curated static list (dim '(examples)')
  when no fresh cache exists — model ids churn, so ANY id the agent's CLI accepts works
  either way (coop never validates --model). A model is an axis of its own — set it with
  --model or a preset's models: ladder, never on a credential.
  ...
  best-effort: an unavailable CLI, timeout, or parse error degrades silently to the last
  cache or the static list — it never errors or hangs.
