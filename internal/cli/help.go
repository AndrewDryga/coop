...
var commandHelp = map[string]string{
  ...
  Each member gets only the MINIMAL set — its task queue, backlog, and project.yaml —
  since members share the root's AGENTS.md, skills, rules, hooks, and box. A single
  repo gets a project.yaml template with commented serve/subprojects examples. The
  .gitignore ignores .agent/ state at any depth (**/.agent/*) and keeps project.yaml
  committed everywhere (!**/.agent/project.yaml). Never clobbers existing files.`,
