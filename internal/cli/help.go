...
var commandHelp = map[string]string{
  ...
  work to — each role an agent: target (provider[:model][/effort]) + routing hints. The lead's
  ...
  .agent/presets/frontier/preset.yaml:
    ...
    roles:                                  # agent: is a target — provider[:model][/effort], default account
      ...
      critic:                               # read-only peer via coop-consult
        mode: consult
        agent: codex:gpt-5.6-sol/xhigh      # omit :model/effort for the agent's defaults
  ...
  depth (**/.agent/*) and commits knowledge — rules/skills/presets/loop.yaml — at any depth
