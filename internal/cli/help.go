...
var commandHelp = map[string]string{
  ...
  Usage: coop acp <agent>[:model][/effort][@account] [--peer <agent>...]
         coop acp <preset> [--peer <agent>...]
         coop acp fusion <agent>[:model][/effort][@account] --peer <agent>...
  ...
  whenever you want to choose a Provider or Account manually. Fusion offers None only while
  at least one explicit non-self peer would remain — a role-only council cannot become solo.
  ...
  replay, not a container + adapter cold-boot. Fusion skips these plain boxes: it has no
  Provider dropdown and preset rungs carry exact targets. Set COOP_ACP_WARM=0 to disable
  ...
  and file contents, so treat it as sensitive.`,
	...
	"fusion": `coop fusion <agent> --peer <agent>... — one agent leads, council members advise, it synthesizes.

  Usage: coop fusion <agent>[:model][/effort][@account] --peer <agent>... [args...]
         coop fusion <preset> [--peer <agent>...] [args...]
  ...
  coop fusion claude --peer codex:gpt-5.5 --peer gemini.
  Each explicit peer is a unique, signed-in provider target; the terminal governor cannot
  also be its peer. Preset members are invoked by role name, so several roles may use one
  ...
  the complete lead ladder and validates the effective council on every reachable provider.
  ...
  including 'coop fusion <preset>', pins and reports the first rung.
