...
var commandHelp = map[string]string{
  ...
  COOP_TASKS). Each member gets ONLY its own task queue + backlog (no project.yaml, no
  AGENTS.md/.claude/rules — those are the root's, shared); the member's queue is for
  its own work, the root's for changes spanning members. A single repo gets a
  project.yaml template with commented serve/subprojects examples. The .gitignore
  ignores .agent/ state at any depth (**/.agent/*) and keeps the top-level project.yaml
  committed (!.agent/project.yaml). Never clobbers existing files.`,
