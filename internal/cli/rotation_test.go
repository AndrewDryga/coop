...
// rts builds a rotation from bare credential names (model empty), for the engine tests.
func rts(creds ...string) *rotation {
	ts := make([]runTarget, len(creds))
	for i, c := range creds {
		ts[i] = runTarget{credential: c}
	}
	return newRotation(ts)
}

// signInCred writes a fake credential for agent so box.ProfileAuthed sees it signed in.
...
func TestRotationSingle(t *testing.T) {
	...
	if sleep <= 0 || !until.Equal(reset) || r.active().String() != "only" {
...
func TestRotationStickyThenWaits(t *testing.T) {
	now := time.Unix(1000, 0)
	r := rts("a", "b", "c")
	if r.active().String() != "a" {
		t.Fatalf("start active = %q, want a", r.active())
	}
	if sleep, _ := r.onLimit(now.Add(2*time.Hour), 1, now); sleep != 0 || r.active().String() != "b" {
		t.Fatalf("after a limited: sleep=%v active=%q, want 0 + b", sleep, r.active())
	}
	if sleep, _ := r.onLimit(now.Add(time.Hour), 2, now); sleep != 0 || r.active().String() != "c" {
		t.Fatalf("after b limited: sleep=%v active=%q, want 0 + c", sleep, r.active())
	}
	sleep, until := r.onLimit(now.Add(3*time.Hour), 3, now)
	if sleep <= 0 || !until.Equal(now.Add(time.Hour)) || r.active().String() != "b" {
...
func TestRotationUnknownResetBacksOff(t *testing.T) {
	now := time.Unix(1000, 0)
	r := rts("a", "b")
	if sleep, _ := r.onLimit(time.Time{}, 1, now); sleep != 0 || r.active().String() != "b" {
...
func TestRotationSameModelFallback(t *testing.T) {
	...
	r := newRotation([]runTarget{
		{model: "fable", credential: "work"},
		{model: "fable", credential: "personal"},
		{model: "opus", credential: "work"},
	})
	if r.active().String() != "fable@work" {
		t.Fatalf("start = %q", r.active())
	}
	if sleep, _ := r.onLimit(now.Add(time.Hour), 1, now); sleep != 0 || r.active().String() != "fable@personal" {
		t.Fatalf("fable@work limited → fable@personal: sleep=%v active=%q", sleep, r.active())
	}
	if sleep, _ := r.onLimit(now.Add(time.Hour), 2, now); sleep != 0 || r.active().String() != "opus@work" {
...
func TestExpandLadder(t *testing.T) {
	...
	if want := []string{"opus@personal", "opus@work"}; !slices.Equal(members(got), want) {
	...
	if want := []string{"fable@personal", "fable@work", "opus@work"}; !slices.Equal(members(got), want) {
		t.Errorf("ladder = %v, want %v", members(got), want)
	}
	// Empty ladder → default model (empty) across accounts.
	got, _ = expandLadder(cfg, "claude", nil)
	if want := []string{"personal", "work"}; !slices.Equal(members(got), want) {
	...
	if slices.Contains(members(got), "opus@ghost") {
...
func TestApplyTarget(t *testing.T) {
	...
	r := newRotation([]runTarget{{model: "sonnet", credential: "work"}, {credential: "other"}})
	a.applyTarget("claude", r)
