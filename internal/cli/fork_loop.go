...
func (a *app) runForkLoop(repo, ws, name, agent, tasks, credential, model, effort string, peers []agents.Target, detached bool) (int, error) {
	...
	code, err := a.loop(ws, img, agent, name, rot, forkQueue, sink, peers, false, false, 0) // name labels each box (coop.fork=); detached/fork loops aren't interactive; no pre-flight/task limit
	if err == nil && !detached {
		forkNextSteps(name)
	}
	return code, err
