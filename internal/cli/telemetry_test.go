...
func TestBuildStageRecord(t *testing.T) {
	...
	rec := buildStageRecord("run123", "signoff", "v4.0.0", tgt, start, end, 0, 3, 2, "abc123", "def456", q)
	...
	for _, want := range []string{`"stage":"signoff"`, `"provider":"codex"`, `"reopened":2`, `"queue_done":5`} {
