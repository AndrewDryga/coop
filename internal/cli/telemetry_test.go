...
func TestBuildStageRecord(t *testing.T) {
	...
	if len(rec.Finished) != 1 || rec.Finished[0] != "t-1" || len(rec.GateFiles) != 1 || rec.GateFiles[0] != ".claude/settings.json" {
	...
	for _, want := range []string{`"stage":"work"`, `"outcome":"success"`, `"provider":"codex"`, `"reopened":2`, `"finished":["t-1"]`, `"gate_files":[".claude/settings.json"]`, `"queue_done":5`} {
