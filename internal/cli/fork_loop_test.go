...
// (naming the holder), and release — the same shape as lockLoopCheckout's own tests above, since the
// spec mirrors its keying exactly. A stuck holder must fail fast (non-blocking), never wedge the
// caller — see the contended case below.
...
// "exclusive" lock on a different inode. Mirrors tasklease_test.go's identical race on the
// task-lease registry (TestLeaseAuthorityLockRechecksInodeIdentity / replaceLeaseAuthorityRecord).
