...
- **ACP automatic startup and live selection are restored.** Editors can use `["acp"]` again:
  ...
  filtered networking, an unrelated signed-in Gemini or Grok no longer breaks Claude/Codex
  startup; selectors and replacement boxes share the same admitted provider scope, including
  complete preset ladders and roles. Unsupported explicit requests still fail closed. Cancelling
  a queued prompt clears its retry without losing conversation or model settings; switching,
  reloading, or disconnecting interrupts obsolete quota waits before another box can start. Process
  tests cover literal no-target startup, all provider switches, cancellation and restart; the
  opt-in live suite exercises all twelve directed provider pairs and retries credential cleanup
  only after its producer processes and boxes have stopped.

- **`coop net` answers in the words a person would use, and every one of its eleven views is a pinned transcript.** Bare `coop net` used to open with a project header and its own path, then `New runs use filtered internet access because .agent/project.yaml says so`, a Claude-and-Anthropic example, `Everything else is blocked` and a `This project can also reach:` list; the family page was a one-screen essay about gateways, checksums and authority stores. Every view was rewritten against approved fixtures. Bare `coop net` is the mode and why, with the YAML explanation used ONLY where the project file actually selected the mode — a stored approval says `because that is what you approved for this project`, a host setting `because COOP_EGRESS says so`, the built-in default `because that is Coop's default` — then `APPROVED NETWORK RULES` when there are any and where to read more; a provider bundle is included automatically and is never listed as something this project asked for. A pending request replaces all of it: `⚠ New runs need your approval`, the actual cause six spaces in, the mode diff and rule diff, and `Review changes: coop net approve`. `coop net runs` shows this project's 25 newest runs under a `RUN`/`WHEN`/`ACTIVITY` header, with `Showing 25 of 33 · coop net runs --all` only when there are more, and rows that keep measured zero (`no external connections`), an unmeasured count (`connection count unknown`), a truncated denial list (`at least 8 blocked attempts`) and a missing receipt (`final record missing`) apart. `coop net check` collapses to one sentence with its rule source — `✓ example.com:443 is allowed by an approved project rule.`, `✗ example.com:443 is blocked — no approved rule allows it.` with the edit-and-approve action, `✓ api.anthropic.com:443 was allowed by Claude's provider access in run e644f07a.` — and no trailing disclaimer, fingerprint or bundle version. `coop net watch` opens with `Press Ctrl-C to stop watching.`, appends one line per new event and closes with the shared final report exactly once. `coop net recover` reports what it actually removed, counts a resource it found already absent as settled rather than newly removed, and states an external blocker in the operator's words (`Docker is unavailable` / `Reconnect to the original Docker daemon`). Every run reference that cannot be resolved is refused through the shared input-error block with the prefixes that would settle it. Breaking: `coop net explain <host>` is now `coop net blocked <host>` — same evidence, same `--run` and `--json`, no alias — and `coop net export --include-destinations` is now `--include-addresses`, because what it puts back are remote hostnames and IP addresses.

- **A box's network summary is the same renderer as `coop net inspect`, under `Networking stats:`.** The summary after an interactive filtered run repeated the run's opaque 32-character id in its heading and then listed every resolved peer beneath each destination — detail for a record you chose to open, not for the run you just watched. The box now prints `Networking stats:`, the allowed aggregate, and each destination with its own totals on its row (`example.com:443 · TLS · 773 B sent · 5.3 KB received`), then the same exceptions in the same order and the same `Full details: coop net inspect <id> --json` footer. It is one renderer with one flag, not a second formatter: the standalone view keeps its `Network run <id>` heading and its per-address rows. The refusal exception now says `⚠ Traffic to 1 remote address was blocked` and points at `coop net blocked <host> --run <id>`, and the launch section that applies the selected access is `Configuring network access` in every mode — `✓ Applied 3 approved network rules` counts normalized approved project rules, not expanded addresses — because a config file supplies the rules and is not the component enforcing them. The network vocabulary is swept through help, output, generated `project.yaml` comments, `docs/networking.md` and the family's own Go names: network access for the mode, network rule for an approved selector, network traffic for what a rule permits, remote address for a recorded hostname or IP. "Website" is gone as a synonym for an IP or ICMP rule.

- **`coop net forget` now fails closed, instead of quietly restoring unrestricted access.** Deleting a project's saved approval did not universally require a fresh one: with the project's `box.egress` request also gone, mode resolution fell through to coop's built-in default, which is open — so withdrawing trust could widen access. Forgetting now writes a trusted withdrawal marker beside the approval and fsyncs it BEFORE the grant is cleared, so the only state a crash between the two can leave is the approval that was already in force. While the marker stands, every ordinary launch of that project — with the YAML removed, with no rules, with a filtered or open request, from `coop run` or the session API alike — stops at `New runs need your approval`, and only an explicit `coop net approve` at a terminal removes it, including an approval of an unchanged or empty request. An offline run and a host-owned named policy are untouched: neither can regain anything. The confirmation says what it does in those words — the approved mode and its rules, then what does not change, what stops and what is kept — and answering no prints `Cancelled. Network approval was kept.` and exits 0. Recovery and repair never lift the barrier: `coop net recover` opens retained evidence and no approval at all.

- **Accounts, presets and models answer the question you asked them.** `coop credentials codex personal` used to print `Codex / personal`, `refreshed 7 hours ago`, `default yes` and the internal directory its token sits in — four facts, none of them something you can do. It now says which account runs (`Codex uses personal by default.`, or `Grok uses this account's configured environment key.` where the authority is your env file) and then the commands that act on it: `Use this account:`, `Use it by default:` when it is not the default, `Sign in again:` unless an env key carries it, `Manage accounts:`. A broken account leads with what is wrong and the exact login that fixes it, and no view prints a storage path. The overview keeps its measured columns and marks only the exception — `⚠ Sign in again`, `⚠ Not signed in`, each with its `coop login <agent>@<account>` beneath, an env-backed account reading `environment file` instead of an invented age, and a default that points at an account nobody created saying `⚠ Default account "personal" is missing` with the command that picks another. Choosing a default reports the choice alone (`✓ Codex will use work by default`, or `Codex already uses work by default.`) instead of re-printing the whole listing, and warns — without refusing — when the account you chose has no login yet. Removing one leads with what is permanently lost before it asks: `Permanently delete Codex account "old-work" along with its saved login and local session history?`, then the shared gate's `Continue? [y/N]` defaulting to No, `✓ Deleted Codex account "old-work"` only after you agree, `Cancelled. Nothing was removed.` when you do not, and — piped with no `--yes` — the shared `✗ Confirmation required` block pointing at `coop help credentials rm`. A failed delete says it could not finish rather than claiming nothing was removed.

  `coop presets` became a comparison you can read: two sentences saying that a preset is several agents with one lead, then `PRESET / FIRST LEAD MODEL / ROLES / SOURCE` — the first configured lead (the whole fallback ladder lives in the detail), the roles counted by what they actually do (`2 advisers, 1 editor`, and `none` when a preset is one agent), and where the file came from, project or global. A preset that will not load keeps its own block with the file and the problem in plain sentences (`Prompt file "roles/critic.md" was not found.`), and the runnable example underneath is never the broken one. An empty project says `No presets yet.` and how to make one. `coop presets <name>` and `coop help <name>` (still one renderer) open with RUN IT — each of the three commands with what it does — then the lead's `Agent:`/`Prompt:` aligned on one gutter, with a long ladder wrapping under the Agent value instead of turning into unlabeled model rows. `coop presets init` ends in ordinary prose: the template it wrote, and `coop <name>` to run it; an existing preset is answered with `Show it:` and an incomplete folder with `Inspect that folder before creating the preset again.` — neither is merged into or repaired.

  `coop login claude@work` says `Signing in to claude@work` and hands the screen to Claude, and claims nothing the provider did not deliver: `✓ Signed in to Claude as work` with `Start Claude: coop claude@work` only when a usable credential was actually persisted, `⚠ Claude exited, but Coop could not confirm sign-in` when it was not, `Sign-in stopped.` when you interrupt it, and no storage directory in any of them. `coop models` keeps its menu and moves a failed refresh's reason into the shared cause block — `⚠ Could not refresh — showing the list saved yesterday` over `Codex is unavailable on this host.`, `Docker is unavailable.`, or `Sign in to Gemini to refresh its models: coop login gemini`, and no paragraph at all when the failure says nothing useful; a signed-out ACP provider is named before a container is started for a fetch that could only fail. Its footer example is a model you can see on the screen — the standing default, else the first id listed.

  Every rejected target, account and preset now renders through the one shared refusal block, with the rule that was broken and the page that explains it: `Invalid agent target "claude:"` over `Add a model after ":" or remove the colon.`, `Gemini does not support a reasoning-effort setting`, `Invalid account name "../work"`, `"coop claude" takes one account` (account lists belong to `coop loop`), `A peer uses its default account`, `No Codex account named "work"` with `Accounts:`/`Sign in:`, `Codex needs a usable account` where a run truly requires one, `No preset named "frontire"`, and `Could not load preset "review"` with the file and the loader's own sentences. The unauthenticated heads-up before an interactive run stays a warning, not a refusal. `coop help login`, `coop help credentials` (plus its `default`, `rm` and account pages), `coop help models` and `coop help presets init` are the approved pages, pinned byte-for-byte in `internal/cli/testdata/approved` together with every state above, and the four provider pages now carry UPPERCASE reference headings like the rest of the manual.

- **The box says it has stopped only once it actually has.** The teardown line used to print the moment the main process returned — before the gateway, the sidecar volumes and the run receipt a filtered box owns had been removed — and it read `coop: stopping the box — main process exited with status 3`, a progress event dressed as a result. It is now one sentence, printed after the stop is confirmed: `The Coop box has stopped — main process exited with status 3.` A plain box is `--rm`, so its client returning IS the confirmation; a filtered box waits for the cleanup that removes it, and a cleanup that could not finish prints no completed-stop claim at all — the error it returned is the whole story. When that teardown outlasts two seconds, `Stopping the Coop box…` says what the silence is, and it is cancelled the moment the box is gone, so the two can never both be true. The reason stays the one the run actually produced — the host signal that cancelled it (`interrupted by Ctrl-C`, `stopped by SIGTERM`), the status the main process returned, or the failure that ended it — and is never inferred from an exit code. The first section of a launch (`Protecting secrets`) no longer opens with a stray blank line: a narration knows whether it has opened a section yet.

- **`coop worker` is retired; one command connects this machine.** Connecting a machine to a fleet controller meant running two things in two terminals — `coop sessions serve` for the local session service, then `coop worker connect --config <absolute-path>` beside it — and getting the order wrong left the connector polling a socket nothing served. `coop sessions connect --config <path>` is now the whole workflow: it validates the configuration FIRST (a bad file never starts a service), then uses the ready local service or starts one, proving readiness through the same `/readyz` check `coop sessions doctor` makes rather than treating a spawned process as a connection. A service that is listening but NOT ready is not absent: its socket is never unlinked and no second service is started, because unlinking a live socket cuts off whoever is already talking to it. Two invocations racing the same session storage resolve through the store's own exclusive lock — the loser re-checks readiness and uses the winner. Ctrl-C stops the connection and, only if this invocation created it, the service it started; one that was already running is left exactly as it was found. Two optional fields in the strict worker JSON, `session_state_dir` and `session_policy_path`, say which local service a configuration is about (defaults `~/.local/state/coop/sessions` and `~/.config/coop/session-policies.yaml`); the existing `coop_socket` must resolve inside that state directory, the state root is never inferred from the socket's parent, and no policy file is ever generated. `coop sessions serve` is unchanged for running the service on its own. Breaking: `coop worker` is gone from the parser, help, the manual and shell completion, with no alias — `docs/worker.md` is folded into `docs/session-api.md`.

- **`coop sessions policies` answers what a remote application may ask this machine to do.** It used to print two hex digests and a fingerprint per policy — `Policy digest: …`, `Authority digest: …`, `Fingerprint: unresolved on this host — …` — which is verification data for a worker configuration, not an answer for the person deciding whether to allow it. The human view now leads with the configuration's name and, under one label column, the project it may use, the file access it grants (`Read-only`, `Read, edit, and commit in a separate project copy`, or `Questions and answers only; no project files or tools` — derived from the policy's mode and repository flag, since there is no separate commit permission), the agents and accounts pinned when the policy loaded, and the network rules this host actually resolves for it: the provider endpoints a selected agent brings with it, then each approved destination as `github.com:443 · TLS`. A configuration whose network this host cannot resolve raises that as an issue — `⚠ Network access needs approval` with the reason and `Run coop net approve in <its project>` — instead of printing a partial rule list that would read as its complete access. Every identifier stays in `--json`, whose fields, digests and bytes are unchanged.

- **Session data compaction, the session doctor and `coop sign` say what actually happened.** Compaction reported `compacted 120 turn retry receipt(s) (37748736 -> 49152 bytes)` and two more raw byte arrows; it now leads with `✓ Session data compacted` and labels the two separate facts — the logical record bytes the rewrite saved and the allocated database bytes VACUUM reclaimed — beside the verified backup and its size. Zero legacy records is no longer reported as nothing having happened: the backup was still written and verified. A failure now names the stage it reached from what the run PROVED — a verified backup exists only once its path is set, and only a post-commit marker proves the receipt rewrite landed, so an accumulated count never implies a committed transaction. The doctor's success is `✓ Session service is ready` over the socket it checked; its failures distinguish a socket nothing is serving from a service that answered but cannot take sessions, and a socket you named keeps its own remedy instead of being answered with a command that would listen somewhere else. `coop sign` says `✓ Signed 3 unpushed commits with your host key` for the default range and `✓ Signed 3 commits with your host key` for an explicit `--from`, which it trusts rather than calling its commits unpushed; `No unpushed commits to sign.` and `No commits after origin/main to sign.` replace `nothing to sign`. Its refusals name what the signer found — no upstream, a merge commit in the range, no branch checked out, a branch that moved while signing — each with the one action that resolves it, instead of diagnosing every failure as a signing-key problem.

- **`coop prompt` reads as a status line, not a debug dump.** `3 todo · 1 blocked · 1 looping · 2 forks · unsigned` counted running loops beside forks as if they were separate populations, and `doing`/`unsigned` were internal words. It is now `2 todo · 1 in progress · 1 blocked · 3 forks (2 running) · unsigned commit`: running loops qualify the fork count, and both remaining words say what they mean. Empty segments are still omitted and an idle project still prints nothing at all.

- **The launch's conditional work is reported in the same shapes as everything else.** The sidecar services, the box image, the published ports and every best-effort step around a launch each had their own sentence — `services: … — continuing without them (run 'coop up' to retry)`, `note: .agent/Dockerfile is untracked in git — …`, `Serving box :3000 at http://localhost:26716`, `Host port 26716 (for :3000) is in use — not publishing this box`, `sandbox activity … cleanup failed: … — work result preserved; inspect 'coop tasks watch --json'`. They now use the two shared blocks: `Starting project services` listing what Compose actually resolved and `✓ Services started`; `Available on this host` with `http://localhost:3000 → box port 3000`, keeping both ports because they can differ; and an amber block with a six-space cause and a two-space action for everything that did not stop the launch — services that could not start or were skipped because another box is running, ports that could not be published, a box with no network to publish on, an untracked project Dockerfile, a global Git ignore that could not be copied, a project environment that was not loaded, task updates that are unavailable, and activity that could not be finished recording. The orphan sweep reports its verified count in the same voice: `Removed 2 boxes whose Coop processes had stopped`.

- **The editor no longer reads Coop's internal vocabulary.** ACP's messages said `coop: authentication failed for claude@personal — run: coop login claude@personal`, `coop: personal is rate limited — switched to work; resend your last message`, `Waiting for a reset on credential personal in 18m (at Fri 10:20 CEST) — your message will send automatically`, and prefixed nine more with `coop:`. They now say what the editor's user should do: `Sign in to claude@personal on the host: coop login claude@personal`, `Account "personal" reached its usage limit. Switched to "work". Send your last message again.`, and `Waiting for account "personal" to reset its usage limit at Fri 10:20 CEST (in 18 minutes). Your message will send automatically.` — a wait that resends by itself never also tells you to resend, and authentication is never described as a rate limit. The first connect's image build is `Checking the Coop box` with `✓ Box ready`, and a stopped Docker is now detected before the build so it is reported as `✗ Could not prepare the Coop box` / `Docker is unavailable.` / `Start Docker, then reconnect your editor.` rather than a build you should wait through. A session Coop could not restore says which of the two happened — the saved state could not be read (a new session starts) or the agent could not reopen it (start a new one yourself) — with the provider's actual cause. The ACP trace names its file the same way whether it opened, could not be opened, could not be protected or could not be rotated. JSON-RPC payloads, ids, events and retry decisions are unchanged.

- **Every page in the runtime family is the page the review approved.** `coop help run`, `shell`, `acp`, `sessions` and each of its five leaves, `sign`, `completion`, `prompt` and `version` are rewritten to the approved copy: UPPERCASE reference headings, the `Usage:` label, options grouped by what they do, and a labeled `For more details see:` pointer instead of an orphan command. They end themselves, so the generic `Run 'coop help' for all commands.` footer is gone from all of them. A leaf page registers under its full path (`sessions serve`) in the same map its family uses, so `coop help sessions serve`, `coop sessions serve --help` and `coop help --all` read one source and cannot disagree. `coop completion` now documents a user-controlled destination — `~/.config/coop/completion.zsh`, sourced after `compinit` — instead of assuming `fpath[1]` is writable, and the generated scripts' comments name that same path; their executable bodies, including the command-local `nocorrect` alias, are unchanged. The generated `coop-consult` and `coop-delegate` guidance inside the box follows the same shape, naming a peer or role this run actually has rather than an example that does not exist here.

- **The command menu is grouped around what you are doing, and every rejected input is refused the same way.** `coop` and `coop help` used to open on `AGENTS`, `CREDENTIALS, MODELS & PRESETS`, `THE BOX`, and rows written for someone who already knew the product (`coop <target>` — "agent target in a box"); a newcomer with no account got a one-line `FIRST RUN  set up in order:` recipe, and a project with no services got a dimmed `none in .agent/compose.yml yet`. The menu now opens with `THE BOX` and reads in the order you actually meet Coop — the box, running agents, accounts and models, tasks, loops, forks, services, security, setup, integrations — with each UPPERCASE heading carrying one plain explanation that names the real file (`TASKS — each task is a folder in .agent/tasks/`, `LOOPS — … configure the steps in .agent/loop.yaml`). With no usable account it opens with `GET STARTED — sign in, then start an agent`, the two commands that begin the day. The SERVICES heading names this project's own services and Compose file (`SERVICES — postgres and redis, defined in .agent/compose.yml`); with none configured it keeps the generic sentence, dims the whole `coop up`/`coop down` rows and leaves `coop init --services` bright, so the one row that helps stands out. It closes with `Help and examples: coop help <command>` and the docs link at column zero. A pipe or `NO_COLOR=1` gets the same text with no escape sequences, and the generated reference (`coop help --all`, `docs/cli.md`, `docs/man/coop.1`, `site/llms.txt`) stays byte-identical on every machine.

- **Rejected input gets one shape, whichever command refused it.** Every parser used to write its own sentence: `✗ unknown command "doctro"; did you mean "doctor"? — run 'coop help' for the list` plus an unconditional `run it in the box: coop run -- doctro`, `✗ --agents needs a value`, `✗ version takes no arguments (got "extra")`, `unknown tasks command "watxh" — use: ls, lint, add, claim, …` dumping every verb. They now build a `ui.UsageError` from data and one renderer prints it: a blank line, a red `✗` headline naming the FULL rejected command including `coop`, an optional cause indented six spaces, then aligned `Did you mean:` / `See available commands:` / `Usage:` / `Example:` / `Help:` rows two spaces in. `coop doctro` says `Unknown command "coop doctro"` and `Did you mean: coop doctor` — with no offer to run a typo in the box; `coop models --refesh` is an OPTION error now rather than an "unknown agent", and corrects to the whole command `coop models --refresh`; `coop init --agents cursor` explains `Choose claude, codex, gemini, or all.` and shows a valid example plus `coop help init`; `coop tasks add` and `coop tasks claim` share one `Usage:`/`Help:` template instead of a per-argument sentence; `coop claude --readonly --bare` says `Cannot combine "--readonly" and "--bare" in "coop claude"`. Stdout stays empty, the status stays 2, and plain output carries no escapes. The exact bytes of all 22 approved transcripts are committed under `internal/cli/testdata/approved` and compared byte-for-byte by the gate.

- **`coop help <command> <subcommand>` and `coop <command> <subcommand> --help` reach the same page.** `coop help` passed only its first word on, so `coop help tasks add` printed the whole tasks guide and a registered agent's `--help` fell through to the provider's own CLI. Help now resolves the command path it was given, from either spelling, with the same order as execution: built-in command, registered agent, then preset. `coop claude --help` is Coop's page for running Claude — the flags it reads, its models and accounts — while `coop claude -- --help` still reaches Claude's own help untouched, and the rule holds for a full target (`coop claude:opus/high@work --help`). A mistyped subcommand is refused against its family's real verb list (`coop help tasks watxh` → `Did you mean: coop help tasks watch`) instead of silently falling back to the family page, while an open slot — a fork name, an account — is never mistaken for a typo. `coop help --all` assembles the complete manual in the menu's own order, every page behind the same rule-off separator, and now includes `coop sign`, which the manual had been missing. Contributor build and test guidance moved out of the user's manual to README's Layout & development, where a contributor looks.

- **Coop's own output no longer prefixes itself with `coop:`.** The bold-cyan anchor was on progress lines, the box's stop line, the network summary after a session, the in-box supervisor's warnings, the serve-port notices and the self-update notices — 87 call sites through `ui.Info`, plus a `Prefix` field the network report carried purely to stamp it after agent output. `ui.Info` is retired and the prefix is gone from all of them: a line reads as the sentence it is. Machine surfaces are untouched — `--json`, the ACP/JSON-RPC payloads, the generated `mcp.json` comment and the `coop: re-sign commits` git reflog message all keep their identifiers.

- **`coop tasks` reads as the work, not as the bookkeeping — and `release` hands the task back to the queue.** The listing led with lifecycle order and closed with a rule and a recount of what you had just read; it now opens with `Tasks`, groups by `IN PROGRESS · 1` / `TODO · 1` / `BLOCKED · 1` / `DONE · 1` in the order work actually moves, and gives each task at most three lines — its title, only what is exceptional about it (`2/4 subtasks`, `Needs your answer`, `claimed by codex (PID 812)`, `Reserved by codex`, `⚠ Owner process has stopped`), and its id. An ordinary todo task and an unclaimed, unreserved in-progress task now say nothing at all instead of `unleased`. The one footer left is the action: `Answer blocked tasks: coop tasks decisions`, and only when something is blocked. An empty queue says `No tasks yet.` with the command that fills it; an empty filter says `No blocked tasks.` and stops. Every result names what happened to which task — `✓ Created task: Fix login retries` with the file it wrote, `✓ Claimed task:` with `Claimed by:` (omitted when the claim binds to no process), `✓ Completed task:` with the archive folder, `✓ Blocked task:` with its decision.md, `✓ Unblocked task:`, `✓ Deleted task:` — and a deletion now leads with what is permanently lost and the exact folder before it asks. `coop tasks block` takes the whole request inline: `--question`, repeatable `--option` and `--recommendation` write decision.md as the task is parked, validated before anything moves (in a monorepo too), never overwriting a decision somebody already wrote — the identical request again is a no-op, a different one names the file to edit. `coop tasks release` no longer just drops your claim: it returns the folder to `00_todo/` with its instructions, `state.md`, `log.md`, decision history, evidence and `tmp` intact, under the same authority and owner locks `block` uses, refusing a fork-owned task, another controller's live lease and a claim another live process holds; the move happens while the claim is still held and the claim is dropped after, so an interrupted release is finished by running it again. An unclaimed todo task is a no-op, and a blocked or done task is not released. `coop tasks decisions -i` frames each question with a bold cyan rule above and below `Question N of M` that stays visible under `NO_COLOR`, and `coop tasks watch` drops the standalone box inventory (the data still drives activity accounting, auto-exit and `--json`) and ends the live view — only the live view — with `Press Ctrl-C to leave this view.` `coop help tasks` is a map of the family with a FOR AI AGENTS section carrying the full `add` and `block` commands, and every verb now has its own page: `coop help tasks release`, `coop help tasks block`, and the rest.

- **`coop tasks flags` is gone.** The task board's "changes what runs on your machine" acknowledgement is removed outright — no replacement command, no compatibility reader: completion writes no `flags.json`, the board and `coop tasks watch` carry no flag marker, and the snapshot's `host_surfaces` field is gone. Existing `flags.json` files are inert historical data nothing reads, writes or deletes. The shared safeguards that have their own consumers stay exactly as they were: `coop fork review` lists what a fork changes that runs on your machine, `coop fork merge` still refuses one that adds an automatic surface, and `coop check-secrets` still reports the ones in your working tree.

- **`coop init --services` adds services to a project that already has some.** The flag used to be first-run only: on an initialized project it did nothing useful, and the only way to get Postgres alongside an existing Redis was to write the Compose block by hand. `coop init --services` on its own now opens a chooser — `Add services`, the accepted names, Enter for none — and `coop init --services postgres,redis` skips it; either way the edit is ADDITIVE. An existing Compose file is spliced, not re-rendered, so its comments, ports, volumes and hand-written services survive byte for byte, and the result is re-parsed before it reaches disk. A service already configured is reported, not rewritten (`postgres is already configured as db.`); a name the project uses for something else is refused by name with nothing changed (`✗ Could not add postgres to .agent/compose.yml` / `The name db is already used by a different service.`); a Compose file coop cannot read names the line to fix. Nothing is started — `Start services: → coop up` is the next step, not something init does for you. Without a terminal the bare form says so and names the explicit spelling instead of hanging. The generated file's header stopped calling the data throwaway: it says data stays in named volumes when services stop, and names the one command that deletes it.

- **`coop down --delete-volumes` names every volume before deleting it, and `-v` is gone.** Stopping services with `-v` handed Compose a `--volumes` flag and hoped; there was no preview, no confirmation, and a habit of typing `-v` for "verbose" cost someone their database. The flag is now `--delete-volumes`, it resolves the project's volumes from the runtime FIRST, prints each one with what it holds, and routes the decision through the same destructive gate every other unrecoverable delete in coop uses — default No, `-y/--yes` to skip the question, and a refusal (exit 2) rather than a guess when there is no terminal to ask at. A runtime that will not say which volumes exist stops the command instead of deleting against an assumption, and "no volumes to delete" is printed only when the runtime actually said so. Only this project's Compose-created volumes are in scope: an `external: true` volume and a bind-mounted project file are not coop's to remove. The retired `-v`/`--volumes` spelling fails as an unknown flag — it is gone, not hidden. Ordinary `coop down` asks nothing and keeps stored data, and `coop up` now ends with `✓ Services ready: db, redis` from the services Compose actually reported ready, plus the URL of any port it really published.

- **`coop check-secrets` can be told about a reviewed false positive, and stops claiming a clean scan over a file it could not read.** A finding coop cannot be argued out of leaves two options — delete the fixture or ignore the command — so a scan now prints a copyable entry per finding, and `.coopsecretsignore` in the project root excuses the exact findings you paste into it with a reason. An entry is `fp-v1:<sha256> # why`, over the file's repository-relative path, a stable detector id and the matched material: inserting a line above a finding keeps it, changing the value or moving the file invalidates it, two tokens on one line are two entries, and two private keys sharing a BEGIN header get different ids because the whole block is hashed. Nothing machine-specific goes in, so the file can be committed and works in every checkout. A glob, a bare path, a rule name, a missing or placeholder reason, an unsupported `fp-v` version, or the same id with two different reasons is refused with the line to fix; an unreadable or oversized exception file stops the check rather than being read as "no exceptions". Exceptions apply to this check only: fork merge, checkpoint upload and session redaction keep seeing every finding, because a note inside a repository is not permission to move a credential out of it. Separately, a file coop could not READ used to be silently skipped alongside the binary and oversized ones it skips on purpose — so a permission error read as "no secrets found". It is now reported by name and fails the scan, even when everything readable was already reviewed. The report itself leads with the result (`✓ No possible secrets found` and what it covered, `✗ 2 possible secrets found` and where), and the changed files that run commands on your machine follow it under `Review files that run commands` with one sentence each.

- **`coop doctor` reports what it measured, and its totals add up.** The report was a flat list of free-text probe messages under lowercase headings, with a final `✓ all N checks passed` where N was however many rows happened to be emitted — so a probe that died quietly shrank the total into something that read clean. It is now `Checking the Coop box on the Docker runtime` (the real runtime name for other backends) and six sections of stated facts — `✓ .env is hidden`, `✓ The box runs as a non-root user`, `✓ The task channel exposes only its 8 task tools` — with the wording held in one table and the probes speaking only stable check ids. A row can say six different things, because they are six different situations: it held, it did not, it was not checked here (the Alpine stand-in, an unreadable limit), this runtime does not apply that limit, configuration switched it off, or the probe carrying it failed — and the verdict names each one it saw: `⚠ 29 checks passed; 6 could not be checked`, `✗ 31 checks passed; 1 probe failed; 4 checks could not be completed`. A failure states the measured fact beneath it and what to do (`The image uses user ID 0.` / `Set a non-root USER in .agent/Dockerfile…`), never the secret it was looking for. The image is named only when it changes which checks ran — a project image, an explicit override, or the stand-in, which gets its own notice. The abandoned-box survey stays outside the tally and says nothing when there is nothing to report.

- **`coop build` and `coop update` say which image they are building and what it did to running work.** Build progress arrived as `coop:` status lines around the runtime's output, and the recycling summary claimed `restarted 2 supervised sessions` on the strength of having removed their containers. A build now opens with `Building the Coop box`, or `Building the project box` with the configured Dockerfile path and the tag it produces, and ends with `✓ Box image built` plus only the effects the runtime actually reported: `2 supervised editor sessions will reconnect with the new image` (they will; coop removed their boxes, it did not watch an editor come back) and `1 other running box will use the new image on its next start`. An untracked box Dockerfile is flagged before the build, which still runs — it is an explicit human action — with the advice attached to USING the result. A context coop could not assemble is a distinct failure from a build that ran and failed. `coop update` replaces the binary first and then rebuilds in that same, pre-update process, so nothing claims the new binary built the image; a release check that fails is a warning and the box update continues, while a binary that could not be installed is an error that still lets the box finish and exits 1, closing with `⚠ The box was updated, but the Coop binary was not`. `The installed binary was not changed` is printed only when the failure provably preceded the atomic replace. `--check` reports the binary, the local build record's age, and each freshness concern separately with one shared next command; the agent-tool version probe that used to be ignored now says when it could not run, instead of leaving an empty heading that reads like a verified answer.

- **A session can start from any source inside the repository its policy already authorizes.** `POST /v1/sessions` used to take one special case — `pull_request: {number, head_commit}` — and everything else began at whatever the policy's configured branch happened to be. It now takes one bounded `source` selector instead: `{"kind":"default"}`, `{"kind":"branch","name":"feature/payments"}`, `{"kind":"pull_request","number":514}` or `{"kind":"commit","sha":"<complete lowercase object id>"}`, with an omitted selector meaning `default`. The caller still cannot name a repository, path, remote, URL or raw ref — Coop derives every ref from the operator policy plus that one value, validating a branch name with Git's own rules and deriving only `refs/heads/<name>` or `refs/pull/<n>/head`. Trusted ingress may add `expected_head_commit` to a pull request so a push racing session creation fails closed rather than silently changing the approved source. A policy with no configured `remote` is intentionally local: it keeps local semantics for its own default and refuses the other three. For anything but the default, Coop pins the configured default head AND the selected head, uses their merge base as the review baseline, starts the generated bound branch at the selected commit, and refuses a source with no common history before any workspace exists. The create response, the public session and the review dossier carry an immutable version-1 `source` binding — kind, normalized request, remote identity, default ref and commit, selected ref (`null` for an exact commit) and commit, merge base, admitted tree and resolution time, plus the pull-request number and expected head where that is the kind — and it is journaled into the durable create intent before the fork exists, so a lost response, a restart or a replay of the same idempotency key reuses exact commits instead of landing on a branch that moved; the same key with a different selector conflicts. Breaking: `pull_request` on create and `pull_request_tree` on changes are gone, replaced by `source` and `admitted_source_tree`, which now describes every kind rather than only a pull request. Sessions created before this keep their rows, and one is relabeled as a pull-request source only where its own freshness receipts prove the remote, refs, heads and merge base; the rest stay readable and are never re-resolved or guessed.

- **An exact commit is proven against the remote, not against the cache.** `git fetch <remote> <oid>` answers success *without contacting the server* when the object is already in the local object database — and the policy-owned cache is the operator's own checkout, so it holds their unpublished commits. Selecting a commit therefore contacts the configured remote every time, and a commit that was already cached is additionally anchored to the remote's current advertisement of `refs/heads/*` and `refs/pull/*/head`: a local-only object is refused with the reason, never silently accepted. Nothing falls back to local `HEAD`, a tracking ref, a cached object or a previous receipt when the remote cannot answer, and resolution never checks out, resets, or reconfigures the cache — it only adds objects. Hosted Git must serve object ids below a ref tip (`uploadpack.allowReachableSHA1InWant`, which GitHub sets); a server that refuses fails the create closed.

- **Workers advertise source selection separately from freshness.** The outbound connector reports `repository-source-selector` version `1` only after the session daemon on its Unix socket publishes `repository_source_selector_versions` in `GET /v1/capabilities`, independently of `repository-freshness` version `2`, and removes either claim from a config file that asserts it. A partially upgraded fleet therefore advertises exactly what each daemon supports, and a controller can withhold selector-bound work from the rest. A `create_session` command carries the selector to the daemon unchanged under the same bounds the daemon enforces, refusing a malformed one before any daemon call; a create fence carries the same request, selector included, so it still occupies the create's exact ledger identity.

- **`coop credentials` is the accounts Coop can use, not a status ledger.** Every row used to repeat `signed in` in green and date its token in the internal `rotated 2h ago` vocabulary, with the default marked by a trailing `(default)`. Being listed already says an account is usable, so a healthy row now carries only when its token material last changed, in words a person says out loud — `refreshed 7 hours ago`, `yesterday`, `19 days ago` — under one title, `Accounts available to Coop`. The account a plain `coop <agent>` runs is marked twice on purpose: a quiet accent `*` makes it scannable, and an explicit `default` in a third column explains the accent without a legend; that column is measured across every agent block, so it starts at the same place down the whole listing. A missing, malformed or expired credential replaces the refresh fact with what is wrong and puts the exact `coop login <agent>@<account>` beneath it, and a default that points at an account that no longer exists says so. `coop credentials <agent>` and the single-account view use the same words.

- **`coop init` asks plain questions and reports one outcome.** A first run used to ask about an internal "commit format gate", offer `[go terraform elixir rust]` while its help named `gofmt` and `mix format` as the things it would run, silently drop any answer it did not recognize, and then print every file, link, skill and `.gitignore` edit it made. It now opens with `Setting up Coop`. In a folder that is not a Git repository it says in two sentences why Coop needs Git and asks `Initialize Git here? [Y/n]`: yes creates the repository before the commit hooks are installed, so nothing is left to activate afterwards, while no — or a run with no terminal, which is never asked — creates nothing and ends with the two commands that finish setup. When no language is detected it asks which ones the project will use, saying Coop checks their formatting before every commit; the menu holds only the answers it accepts (`go`, `terraform`, `elixir`, `rust`), several are allowed at once, Enter means no formatting check, and an answer it cannot accept is named — `✗ Unknown language “rustfmt”` — and the question asked again, rather than half-applied. Optional services (`postgres`, `redis`) follow the same rule. The result is `✓ Coop project created`, the agents that now share instructions, skills and one task queue, the formatting checks and services selected, then one paragraph: network access is filtered, each agent can reach its own provider, and other network traffic is blocked until you approve rules allowing it. Below that only the actions this project's actual state needs — finish setup, sign in, build the box, start services — before `coop doctor` to verify the sandbox and the first task, whose `coop loop <agent>` names the agent this project actually set up (a bare `coop loop` has no target in a fresh project and would fail on the first try). `coop help init` is now the matching short contract: what Coop sets up, what a first run asks, what the three options change, and that re-running it keeps your files and adds what is missing.

- **Zsh no longer offers to "correct" `coop codex` into your `.codex` directory.** With `CORRECT_ALL` on, every agent argument collided with the directory Coop scaffolds for that agent. The generated integration has carried `alias coop='nocorrect coop'` for exactly this, but an autoloaded `_coop` on `fpath` registers completion without ever running the file — so the alias has to exist before Zsh parses the line. The setup guidance now says so: generate with `coop completion zsh > "${fpath[1]}/_coop"`, then `source` that file after your `compinit` line. `install.sh` prints those two steps to a Zsh user and edits no startup file of yours; spelling correction stays on for everything else, which the tests prove in a real interactive shell against `.claude`, `.codex` and `.gemini`.

- **`coop models` keeps itself current, and reads as a menu again.** It used to be a labeled block per agent — `Models:`, `Last refreshed: 2h ago`, the env var in a parenthetical — followed by a cache disclaimer and a four-row `one run`/`standing`/`loop steps`/`everywhere` table, with staleness left to you to fix by remembering `--refresh`. Now each agent is its title-cased name and its ids, wrapped to your terminal without ever splitting an id, and the provider's synthetic `default` choice is gone (leaving a model out of a target already selects the default). The footer is two lines you can copy: `Start Claude with a model` → `coop claude:fable`, and `Set models for presets and loops` → `coop help models`; a standing `COOP_<AGENT>_MODEL` reads as `Default for Claude runs: fable` instead of a fake id. Upkeep is coop's job: every invocation refreshes, in parallel, only the catalogs it is about to show that are missing or older than 24 hours, and a refresh that works prints nothing. One that does not keeps the command useful — the last list it did fetch while that is still worth showing, else the release-bundled examples — under one warning on that agent alone, saying which of the two you are looking at and why: `⚠ showing the list saved 2 days ago — could not refresh: Docker is not running`. A failed attempt is recorded separately from the last success, so the next few reads back off instead of paying the same bounded timeout again; `--refresh` ignores both the 24 hours and the backoff. `coop help models` is now the job it does: list, use a model, set reasoning effort, choose an account, the full `<provider>:<model>/<effort>@<account>` form, a default, and what automatic rotation across models and accounts means in plain words — with the cache, ACP and precedence-chain essay gone.

- **Filtered runs set the host up themselves; approval is an exact diff of the project file.** A first `coop claude --egress filtered` used to stop at `run 'coop net setup'`; the launch now performs the same bounded qualification when this host has no current proof — building or reusing the image pair, proving the five checks, recording it — shows the transcript and continues into the box, serialized so two first launches never build the same images side by side. A Docker that is not running, or a failed check, stops the launch with the real reason. `coop net setup` stays as the prepare-ahead and recheck verb, and its transcript is now two sentences (the Docker version and platform, whether the images are reused or built) plus one doctor-style line per property the smoke proved and a verdict: `✓ all 5 checks passed — this host is ready for filtered runs`; a failure keeps the passes, names what happened, claims nothing later, and says no setup was saved. The records directory, daemon id, image hashes, client inventory and timings stay in retained evidence. `coop net approve` lost `--mode`: the mode and rules come only from `.agent/project.yaml`, the review is one stable diff (`already approved` dim, `new request` yellow, `no longer requested` dim, an `open` request in red), the question is `Approve these changes for new runs? [y/N]` and the answer `✓ Approved for new runs`; a file that already matches its approval prints `No approval needed — this project's network access has not changed.` and writes nothing. That same exact comparison — mode, rules and each named service's reviewed definition — is what `coop init` reports as `⚠ This project asks for network access that has not been approved` / `Review it: coop net approve` (only when true), what bare `coop net` shows as the pending diff, and what every launch refuses on before a box starts. A repository can no longer widen itself to `open`: that too is a request a human approves. `coop init` writes `box.egress: filtered` into a new project file (an existing one is never rewritten), and the file spells no-network access `offline` — the only spelling it accepts; bare `coop net` no longer reports setup health, since a launch handles it. Breaking: `.agent/project.yaml` files that say `egress: none` must say `offline`.


- **An interactive launch narrates itself in sections, says why its box stops, and ends with the run it recorded.** `coop claude` used to print isolated `coop:` lines (`shadowed 8 secret path(s)`, a `run 'coop build'` nag on every launch when the image lagged the binary) and then, after the agent, a refusal-only summary that hid the destinations the box actually used. Before the agent starts the launch now reads top to bottom: `Checking the Coop box` only when the shared image was built from a different box definition than the running coop carries — it is rebuilt through the ordinary build path, names both versions and ends with `✓ Box updated`; an age-only nudge never forces an update, a foreign image is never replaced — then `Protecting secrets` with the exact count hidden, one `Internet access` section that in filtered mode lists each selected provider's endpoints, the approved websites/services and the configured MCP servers before claiming `✓ Everything else blocked` (open: `⚠ Unrestricted — nothing is blocked`; offline: `⚠ Offline — Codex cannot reach OpenAI`), and `Starting Codex`, which flows straight into the agent's output. A section that fails ends with a red ✗, the concrete reason indented beneath it and the remedy — start Docker, then repeat the same command — and is not repeated by the dispatcher. When the box stops, one line says why: `coop: stopping the box — main process exited with status 0`, or `— interrupted by Ctrl-C` when a signal reached coop, never guessed from an exit status; the second interrupt still escapes a wedged teardown. A filtered box then prints the same destination-first view `coop net inspect` shows, under `coop: Network run <id>` — allowed traffic and every peer, refusals with `coop net explain <host> --run <id>`, the same exceptions — from one shared projection (`internal/networkreport`), so the old refusal-only summary is gone. Loops, ACP children and quiet probes keep their bounded output; the loop's closing summary points at `coop net runs`.
- **A filtered Claude session reaches its connectors, and a managed client stops talking to itself.** A `coop claude --egress filtered` hello-and-exit run recorded four refusals nobody asked for — `mcp-proxy.anthropic.com`, `raw.githubusercontent.com`, `registry.npmjs.org` and a Datadog log intake — and offered to add each to `box.egress_rules`. Only the first was the login doing its job: claude.ai connectors are on by default and every one of them is reached through that proxy, so it now belongs to Claude's provider bundle (version `2026-09-10.1`; bundle content is pinned per version, so the set moves with a new one). The other three were the client's own release-notes feed, package registry and telemetry, and a host that delivers arbitrary repository content or packages has no place in the access every session receives. Those are switched off at the source instead, in the box only and never in your host profile: Claude runs with `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1` and `DISABLE_UPDATES=1` (its OAuth refresh, inference and connectors keep working on the locked 2.1.260), codex reads a generated `config.toml` that opens with `check_for_update_on_startup = false`, `analytics.enabled = false` and the three `otel` exporters set to `none` on top of your own settings — direct and ACP alike, shared MCP or not — and gemini gets automatic updates, update prompts and usage statistics off in its generated settings plus `GEMINI_TELEMETRY_ENABLED=false` (it stays unsupported for filtered runs). No hostname is filtered out of any report: a later deliberate request to one of those hosts is recorded, shown and approvable like any other.
- **`coop <agent> --readonly` and `--bare`: runs that cannot change anything.** A repository investigation and a bare routing question both used to get the full box — a writable checkout, the credential home, the dependency and asdf volumes, the project's hooks, MCP servers and services — when all they needed was to read, or to answer. Both flags now launch under one restricted filesystem profile: a read-only container root, run-private in-memory scratch (the box home, `/tmp`, and an empty `/workspace` for bare) discarded at exit, every host path read-only, and nothing project-defined loaded. The provider is seeded with the access-only projection of the selected login (refresh authority never leaves the host), its first-run defaults and a note stating the contract, copied into the tmpfs home before it starts. `--readonly` mounts the repo and approved companions read-only; `--bare` mounts nothing and adds the provider's own no-tools switch (`claude --tools ""`), so the model's request carries no tool. The assembled launch is proven against the profile before Docker runs it, and a mode the runtime or provider cannot enforce is refused by name rather than offered on a promise: Docker only, `claude` only, no peers, presets, `COOP_IMAGE`, `--egress filtered` or runtime arguments beyond `-e KEY=VALUE`. `coop run`/`coop shell` take the same flags, which is the deterministic way to probe what such a run can write. Normal runs are byte-for-byte unchanged.
- **The session API runs `readonly` and `bare` sessions.** A policy's new `mode:` selects the same restricted profile for every session it creates: `readonly` pins and forks the repository as before and mounts the fork read-only, `bare` mounts nothing — no Git runs, no fork or workspace exists, and the session row binds a policy and no repository. The adapter runs under the same switches the CLI proves (`--setting-sources user`, `--strict-mcp-config`, and for bare `--tools ""` plus the stated no-tools system prompt), sent on `session/new` the way claude-agent-acp reads them; the box refuses a provider whose adapter has no proven switch, so `claude` only. What a restricted session cannot honor is refused by name at policy load — a bare policy with a repository, remote, branch, companion, `repository_read_only`, `project_env` or `project_mcp`; a readonly policy without a repository; either with `warm_idle_timeout`, `egress.mode: filtered`, or a `codex`/`gemini`/`grok` rung — and at create or submit: a pull request or Responder binding on a bare session, `require_semantic_validation` on either. Each turn of a restricted session is a fresh native session, because the box's provider history dies with it; a schema-invalid structured result is regenerated from the admitted prompt instead of re-prompted. The public session carries `"mode"`, and every repository-specific operation (changes, checkpoint, restore, workspace task) answers a bare session with `invalid_session_state` (409) `session has no workspace: its policy is bare`; close and discard still work, and a bare box is reaped by its run receipt alone. The mode is immutable session authority: a restarted daemon runs a session under the mode it was created with, and a normal policy's digests are byte-identical to before. `coop acp <target> --bare` and `coop fork <name> acp <target> --readonly` are the daemon's children, usable by hand.
- **The box is told up front which services and ports it actually got.** Sidecars start before the box and an ordinary run continues without them when they fail, but the agent was never told: it found out by failing to connect to a database that was never there and burned turns diagnosing a host condition it cannot see. Every agent's instruction file now ends with a short section stating each sidecar and the address it answers at from inside the box, which serve ports reached the host, and, when the services did not start, that the box runs without them and why. A repo with no compose file and nothing to publish gets no section.
- **`coop net inspect` leads with the destinations a run reached and reports only what went wrong.** It printed a flat status dump for every run — the invariant filtered mode, policy and gateway identifiers, reader timestamps, four healthy layers, zero refusals, `Cleanup complete`, `Receipt final` — and omitted the one useful thing, the destinations the workload actually reached. The human view now shows the allowed aggregate, every proven destination with its port, transport, resolved peers, connection counts and bytes (repeats grouped, distinct peers kept, unmeasured values `UNKNOWN`), then only exceptions: blocked attempts with a hostname-based `coop net explain`, alerts, gaps in the evidence, a layer that did not run normally, a cleanup still owed. Before it reports a cleanup as incomplete it makes coop's own bounded recovery attempt through the existing ownership-checked path and re-reads the run; only an external blocker (Docker stopped, a different daemon) is reported, never an instruction to run recovery. `--json` keeps every field. The `coop net` family is regrouped around the questions a person has — ACCESS (`coop net`, `approve`, `check <url>`, `forget`), RUNS (`runs`, `inspect [<run>]`, `explain <host>`, `watch`, `export`) and REPAIR (`setup`, `recover`) — with unique short run ids accepted everywhere, `inspect` defaulting to the newest run of this project, `runs` replacing `ls` with a bounded five-row view, `check` replacing `why` (current project policy first, `--run` for a recorded run's), `explain` taking the blocked host and printing the exact YAML to paste when the evidence proves it, and `export` replacing the human `receipt` with the redacted shareable JSON. Bare `coop` gains a blank line between its tagline and `Usage:`.
- **A member nested inside a member is discovered too.** `coop init` found a member at any depth but stopped at the first one on each path, so a terraform root that itself holds three roots printed "3 member(s)" against a `.agent/project.yaml` that had to list six by hand — and anyone tidying the file to match init silently dropped three queues, because an unlisted queue is simply ignored. Discovery now walks into a member, registers the missing nested ones in place, and a path that sits inside two members belongs to the nearest one, so no work is routed to two queues.
- **A repo can say which files its services need, and `coop up` labels the prompt with it.** `services.require_real_files` in `.agent/project.yaml` lists the paths whose real contents the sidecars need, so the ask is documented and travels with the repo. It grants nothing, because an agent in the box can edit that file: the human still approves once per machine. What it buys is a prompt you can read at a glance. A file the repo asked for reads as expected, a file nothing asked for is listed first as the one to look at, and a file that appeared since the last approval is called out even when a Compose edit voided the approval that covered it.
- **The notice naming a hidden service file now reaches you on every start.** It was written to the writer coop hands Compose, and a box start points that at a buffer it reads only when Compose fails — so on the one path that matters, a session started a service with an empty stand-in and said nothing, leaving a crash with no explanation. It goes to coop's own status channel now, on `coop up` and on every box start alike.
- **A sidecar's empty stand-in no longer vanishes from under it.** The decoy a sibling service mounts in place of a secret-looking file was created in the private per-start directory coop deletes as soon as the Compose command returns, but `compose up -d` leaves the services running long after that: the container was left bound to a deleted path, and what it saw after a restart was whatever the runtime invented for it. The decoys now live in coop's own state directory under `~/.local/state/coop/decoys`, created once and shared, so a running or restarted sidecar keeps seeing the same empty read-only file. The primary box's decoy is unchanged: that container dies inside the same command.
- **A session's Docker networks are removed with its containers, and leftover coop networks are swept.** Ending an editor session removed the workspace's sidecar containers but left its Compose network behind; every session in a fresh worktree gets its own project, and Docker's default address pool holds only about thirty networks, so a day of sessions ended in "address pools fully subnetted". Session teardown now removes the project's networks that no container uses, and the start of a loop, fork, or build sweeps the networks of coop's own Compose projects that nothing is attached to, across all workspaces. Networks of any other Compose project are never touched.
- **A human can let services read a secret-looking file the box never sees.** Sidecar secret shadowing (below) handed Keycloak an empty decoy in place of its generated dev TLS key, so every `coop run` in that project started a crashing Keycloak. `coop up` in a terminal now lists the secret-looking files a Compose file binds into its services and asks once; the approval is keyed to the file's exact content and stored under `~/.local/state/coop`, so an agent editing the file resets it. It covers exactly the files you saw: a secret that appears later under an approved directory bind keeps its empty stand-in until you approve that one too. Box launches never ask: until approved they start the services with decoys and print which file is hidden and how to approve it.
- **Changes that alter what runs on your machine are highlighted wherever you review agent work.** An agent in a box cannot run anything on the host, but it can plant something that runs when you touch its work later: a commit hook, a `.envrc`, editor or agent settings, an MCP config, `.agent/compose.yml`, a skill, a CI workflow, or the Makefile. Coop now recognizes those files and names the action that runs them ("runs on your machine on `git commit`"). `coop fork review` lists them in a "runs on your machine" section, `coop fork merge` refuses a fork that adds or edits one that runs by itself (hooks, settings, compose, attributes) unless you pass `--force`, and `coop check-secrets` lists the ones changed in your working tree. Deliberate surfaces such as the Makefile are listed and flagged but never block a merge.
- **Sibling services start only when no agent box is running in the project.** Docker opens a bind mount by path, so a running agent could swap a validated bind folder for a link to a host path in the instant between coop's check and the container start — and the only start it could race is one that happens while it runs. Box launches now skip the sidecar start (services already up are still discovered and reachable), and `coop up` refuses while a box is running, naming it. Shared folders stay shared and writable; nothing moves to copies or volumes.
- **The installer fails closed on a missing signature bundle.** With cosign installed, `install.sh` used to downgrade to a warning when a release's `checksums.txt.bundle` could not be fetched, so an attacker able to replace the archive and `checksums.txt` only had to omit the bundle. Every release from v2.2.2 on ships one, so for those a missing bundle now aborts the install; older tags still install on the checksum alone, and a tag that is not a plain version is treated as signed. Decided over verifying the bundle inside `coop update`, which would embed a trust root and pin the release workflow's identity into every binary.
- **The `/sweep` Stop hook leaves another live agent's claimed task alone.** The queue guard counted every in-progress folder as actionable, so a sweep could not end while a second agent (a parallel Codex session in the same checkout) held a task under a live, process-bound claim — work the sweeping agent must neither finish nor block. The guard now skips an in-progress task whose claimant process is alive and outside the hook's own ancestry; a dead claimant, an unbound claim, or the sweeping agent's own claim still counts, and without `coop` on PATH it counts everything as before.
- **Host git never reads a repository's own configuration any more.** Every repository coop runs git against is agent-writable, and git executes whatever its config names: a clean/smudge filter, a textconv or merge driver fired on the host during `status`, `diff`, checkout, or the land rebase whenever an in-tree `.gitattributes` named one — the previous driver neutralizer missed drivers defined in included config and `config.worktree`, and enumerating names first lost the race against an agent writing the config in between. Host git now runs under a coop-owned view of the repository (`GIT_DIR` pointing at a directory whose config coop projects from an allowlist of operational data, with the real objects, refs, reflogs and index behind it), so a repository-defined driver simply does not exist for it. Ref-store and worktree-admin operations run on the real git dir where they belong; the view lives under `~/.local/state/coop/gitviews/`, and an interrupted `coop fork merge` recovers its rebase from there. `core.alternateRefsCommand` joined the blanked knobs.
- **Worker session creation is fenced to the policy it was authorized against.** A worker's create command carries the policy and authority digests the controller pinned it to, but the worker dropped both when calling the daemon, and a daemon restarted with a changed same-name policy would create the session under the new one. `POST /v1/sessions` now accepts optional `expected_policy_digest` / `expected_authority_digest`, the worker forwards its pinned digests, and admission refuses a mismatch with `policy_digest_mismatch` (409) before any intent is journaled or a workspace exists. The worker also reads the daemon's typed error code now (it reported every refusal as `http_error`), so the controller sees the cause. An idempotent replay of a create is held to the same rule: the stored session's authority must equal the one the replayed request names, so a replay is never answered with a session created under a different authority. A create that names no authority at all replays unchanged.
- **Sibling services get the box's secret shadowing.** A validated `.agent/compose.yml` could bind the repo, a directory in it, or a single file into a sidecar, and Compose handed the sidecar the raw host files — the same `.env` or key the box itself only sees as an empty decoy. Coop now adds a generated override to every compose invocation: a decoy over each shadowed path under a directory bind (secret directories whole), and a decoy in place of a bind whose source is itself a secret, symlink aliases included. `.coopignore` applies through the bind; named volumes, tmpfs, and public files are untouched.
- **`coop check-secrets` reports a secret in a file coop shadows by name when git would commit it.** A real private key in `id_ed25519` or a PEM block in `leak.pem` about to be committed was skipped because the box never sees such files — but a push commits them all the same. The default scan now covers name-shadowed commit candidates (findings say so); a file hidden by your `.coopignore` stays silent, and a shadowed file git would not commit (gitignored) stays skipped in both modes.
- **`coop doctor` probes the image the repo actually runs.** It always tested the shared base image, even in a repo with a built per-project `.agent/Dockerfile` image — the non-root, capability, and shadowing checks are exactly what a custom Dockerfile (`USER root`, extra tooling) can weaken, so the proof ran on the wrong box. Doctor now uses the repo's own image when built, falls back to the base image, and only then to the alpine stand-in it already warned about.
- **Sidecars of a read-only session cannot write into its checkout.** Sibling services were auto-started for every launch, including sessions whose repository is mounted read-only, and compose validation allowed read-write binds of the repository — a dev server writing build output, or a hostile compose file on a PR branch, could still write into the supposedly read-only checkout. Such sessions keep their sidecars, but a bind of the repository into one must now be `:ro` (or `read_only: true`); a writable bind is refused with the reason and the session continues without services. Writable sessions are unchanged.
- **A warning-severity typed provider failure no longer fails the turn.** The adapter extension Coop consumes publishes `severity: "warning"` for advisories and retry-in-progress notices; Coop treated any non-error severity as malformed and failed the turn as `acp_protocol_error`, discarding a valid answer delivered beside it. Warnings are now ignored with the stop reason staying authoritative; an error stays terminal, and a severity the extension does not define still fails closed.
- **A workspace checkpoint restore and a session's first turn no longer race.** The restore holds the session runtime from validation through the task binding, so a turn cannot start on a workspace that is being rewritten, and a turn submitted into that window is refused with `invalid_session_state` ("a workspace restore is in progress") before any receipt is journaled — the same idempotency key is accepted once the restore completes. A turn queued first still makes the restore refuse before it touches a file.
- **A task fork can be fixed and re-merged after a red merge gate.** Once a fork's reviewed candidate was published, any new commit was refused ("fork changed after its reviewed candidate was published") and the fork read as drained, so after a red gate the only exits were `fork rm --force` or `--fresh --force`, discarding the reviewed work. Re-entering the fork now retires a candidate the fork's HEAD has moved past (the fix commits descend from it): its owners return to reviewing, the next signoff republishes the new HEAD, and `coop fork merge` lands that. A HEAD that does not descend from the candidate still fails closed.
- **An interrupted fork discard fails closed until it is replayed.** A discard that crashed between removing a task's owner record and its assignment index used to let the fork take new work on the next loop run (index recovery dropped the half-discarded assignment as an orphan), and a later `fork rm --force` replayed only the journaled set, removing the generation under the new assignment. Assigning work to a fork with a pending discard intent is now refused with the replay command, the task-state summary stays readable mid-discard so `--force` can reach the replay, and the replay covers every assignment the fork holds at that moment.
- **`coop fork rm --force` returns the tasks of a fork whose workspace is already gone.** A fork holding an assignment whose workspace directory had been deleted (or emptied with `git clean -fdx`) could not be discarded: reading its task state failed on the missing proposal outbox, and the orphaned-state branch never returned tasks even with `--force`, so its canonical tasks stayed fork-owned forever. A missing workspace or outbox now counts as no pending proposals; `--force` runs the same journaled discard as for a present workspace, and without it the refusal names `--force`.
- **Two narrow fences around moving a task's workspace.** Restoring a checkpoint onto a session already bound to a different task (or base commit) is now refused before any file is touched; it used to reset and rewrite the workspace first and refuse afterwards. And an outbound worker that already holds a newer placement generation for a session now answers a still-leased `submit_turn` or `ensure_workspace` from the older generation with a definite `placement_superseded` failure instead of executing it; checkpoints, artifacts, discard, and close for the old generation still work.
- **An ACP reload survives a failed box cleanup.** On a SIGHUP reload (what `coop update` and `coop build` send every supervisor at once), a transient `docker ps` / `rm -f` failure in the pre-exec sweep used to exit 1 and end the editor session. The reload now warns and re-execs; the next generation retries that sweep once, and the orphan sweep still reaps anything left once the supervisor exits. The final teardown sweep stays fail-loud.\n- **A symlinked settings file or `.agent` directory is refused by name, with the remedy.** Provider settings (`settings.json`, `config.toml`, `.claude.json`) are read without following links, and a dotfile-managed symlink used to stop a launch with "too many levels of symbolic links"; a symlinked `.agent` directory or `project.yaml` was refused as "must be a directory" / "must be a regular file". Each now says the path is a symbolic link and to replace it with a regular file or a real directory, matching the `mcp.json` refusal.\n- **`coop loop` reports usage errors before looking for a container runtime.** A bad `--max-tasks`, an unknown provider in the positional target (`coop loop nope:target` now says `unknown provider "nope" — use claude, codex, gemini, grok`, as `--peer` already did), or a malformed `loop.yaml` used to surface as `runtime "…" not found` on a host without the runtime; the runtime is now detected only once the run is otherwise valid.
- **`coop presets` prints the lead as a real target.** The summary line joined provider and model with a slash (`claude/claude-fable-5`), which no other surface accepts; it now uses the target grammar every rotation surface shares (`claude:claude-fable-5[/effort][@accounts]`), so what you read is what you can run.
- **A backgrounded `coop tasks lease` holder binds to the task's claimant.** An IDE agent starts the holder from a tool-call shell that exits right after the fork, so the holder found no agent above itself, ran unbound, and `coop tasks done` by that same agent was refused with "leased by another controller". A holder that cannot identify its own agent (and was given no `--pid`) now follows the live process bound to the task's claim, says so, and stands down for that agent's `done`/`block` as intended; a person's claim or a claimant that is gone leaves it unbound as before.
...
- **Fork conversation resume state is no longer best-effort.** Coop now refuses to launch until a
  ...
  exact conversation re-entry is unavailable, instead of silently starting over next time.
...
- **The security baseline is enforced locally and in CI.** Coop now builds with Go 1.26.6, runs a
  ...
  edit to the shared source. Shared and native config reads reject symlinks and special files,
  operate nonblocking on one validated descriptor, and stop at 4 MiB. A malformed, ambiguous,
  unreadable, or oversized file, or a `COOP_MCP_FILE` placed inside a directory Coop mounts
  wholesale, now fails before provider launch. Codex and Grok native TOML is also checked
