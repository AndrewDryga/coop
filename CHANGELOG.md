...
- **Changes that alter what runs on your machine are highlighted wherever you review agent work.** An agent in a box cannot run anything on the host, but it can plant something that runs when you touch its work later: a commit hook, a `.envrc`, editor or agent settings, an MCP config, `.agent/compose.yml`, a skill, a CI workflow, or the Makefile. Coop now recognizes those files and names the action that runs them ("runs on your machine on `git commit`"). `coop fork review` lists them in a "runs on your machine" section, `coop fork merge` refuses a fork that adds or edits one that runs by itself (hooks, settings, compose, attributes) unless you pass `--force`, a finished task whose commits touched one carries a red ⚠ on the board until you acknowledge it with `coop tasks flags <id> --ack`, and `coop check-secrets` lists the ones changed in your working tree. Deliberate surfaces such as the Makefile are listed and flagged but never block a merge.
